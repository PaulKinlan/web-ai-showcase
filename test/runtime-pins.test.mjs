...
test("runtime-pin-allowlist.json exists, parses, and has required structure", () => {
  ...
  assert.ok(Array.isArray(data.transformers?.allowedLocalOverrides), "allowedLocalOverrides must be an array");
  assert.ok(Array.isArray(data.onnxruntimeWeb?.allowedVersions), "onnxruntimeWeb.allowedVersions must be an array");
...
test("all @huggingface/transformers versions in the repository are in the allowlist", () => {
  ...
  for (const v of found) {
    assert.ok(allowed.has(v), `unauthorized @huggingface/transformers version "${v}" found in repository`);
  }
});
...
test("scripts/audit-model-currency.mjs --check succeeds on the clean tree", () => {
  ...
  assert.match(out, /PASS — \d+ built routes covered; evidence matches catalogue; runtime pins authorized\./);
});

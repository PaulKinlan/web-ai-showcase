// Route-complete acceptance validator for the MobileViT-Small ImageNet family (apple/mobilevit-small via the canonical Xenova ONNX mirror).
...
// Multi-model is forced down its uncertain path so BOTH MobileViT and ViT execute for real.
...
const SLUG = "mobilevit-small";
const MODEL_ID = "Xenova/mobilevit-small";
const ROUTES = {
  overview: "models/mobilevit-small/",
  basics: "models/mobilevit-small/basics/",
  practical: "models/mobilevit-small/practical/",
  wild: "models/mobilevit-small/wild/",
  multimodel: "models/mobilevit-small/multimodel/",
};
...
async function runRoute(routeName, viewportName, viewport, first = false) {
  ...
  try {
    ...
    if (routeName === "overview") {
      ...
      check(
        `${viewportName} multimodel runs MobileViT AND ViT stages`,
    ...
    await screenshot(cdp, page.sessionId, `/tmp/mobilevit-${routeName}-${viewportName}.png`);
    pass = true;
  } finally {

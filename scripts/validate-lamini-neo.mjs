#!/usr/bin/env node
// Route-complete lamini-neo acceptance: real browser inference on every published route at desktop
// and mobile. Advertised stage driven for real:
//   Xenova/LaMini-Neo-125M  (all routes — text-generation instruction following, WASM int8, 140MB)
...
const RUN_RECORD = join(repoRoot, "models/lamini-neo/acceptance-run.json");
const PROFILE_DIR = mkdtempSync(join(tmpdir(), "lamini-neo-acceptance-"));
if (WRITE_RUN) rmSync(RUN_RECORD, { force: true });

const STAGE = "Xenova/LaMini-Neo-125M"; // the one advertised model stage
const ROUTES = {
  overview: "models/lamini-neo/",
  basics: "models/lamini-neo/basics/",
  practical: "models/lamini-neo/practical/",
  wild: "models/lamini-neo/wild/",
};

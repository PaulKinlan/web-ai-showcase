#!/usr/bin/env node
// Route-complete mms-tts-romanian acceptance: real browser inference on every published route at
// desktop and mobile. Advertised stage driven for real:
//   Xenova/mms-tts-ron  (all routes — MMS-TTS Portuguese VITS synthesis, WASM q8, ~38 MB)
...
const RUN_RECORD = join(repoRoot, "models/mms-tts-romanian/acceptance-run.json");
const PROFILE_DIR = join(homedir(), ".cache", "webai-validator-profiles", "mms-tts-romanian");
mkdirSync(PROFILE_DIR, { recursive: true });
if (WRITE_RUN) rmSync(RUN_RECORD, { force: true });

const STAGE = "Xenova/mms-tts-ron"; // the one advertised model stage
const ROUTES = {
  overview: "models/mms-tts-romanian/",
  basics: "models/mms-tts-romanian/basics/",
  practical: "models/mms-tts-romanian/practical/",
  wild: "models/mms-tts-romanian/wild/",
};
...
async function exercise(cdp, page, rung, viewport) {
  ...
  const SAMPLE1 = "Această voce este generată integral în browserul dumneavoastră, fără niciun server.";
  const SAMPLE2 = "Învățarea limbii române este o călătorie fascinantă prin limbă și cultură.";

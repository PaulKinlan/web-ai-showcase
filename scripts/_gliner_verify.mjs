...
try {
  ...
  const r = await evalL(pg.sessionId, `(async()=>{
   try {
    ...
    const sess = await ort.InferenceSession.create(new Uint8Array(await (await fetch("https://huggingface.co/"+REPO+"/resolve/main/onnx/model.onnx")).arrayBuffer()),{executionProviders:["wasm"]});
    const SPLIT=/\\w+(?:[-_]\\w+)*|\\S/g;
    ...
    async function extract(text, entities, threshold=0.5){
      // whitespace tokenize with char offsets
      ...
      // prompt = <<ENT>> ent .. <<SEP>> words
      const promptItems=[]; for(const e of entities){promptItems.push("<<ENT>>");promptItems.push(e);} promptItems.push("<<SEP>>");
      const promptLength=promptItems.length;
      const seq=promptItems.concat(words);
      // encode
      let wordsMask=[0],inputIds=[1],attn=[1],c=1;
      seq.forEach((word,wordId)=>{
        const enc=tok.encode(word); const sub=enc.slice(1,-1);
        sub.forEach((t,ti)=>{ attn.push(1); if(wordId<promptLength)wordsMask.push(0); else if(ti===0){wordsMask.push(c);c++;} else wordsMask.push(0); inputIds.push(t); });
      });
      wordsMask.push(0); inputIds.push(tok.sep_token_id); attn.push(1);
      ...
      const numSpans=textLength*MAXW;
      const bi=(a)=>BigInt64Array.from(a.map(BigInt));
      const feeds={
      ...
      const inputLength=textLength,numEntities=entities.length;
      const idToClass={}; entities.forEach((e,i)=>idToClass[i+1]=e);
      const startTokPad=MAXW*numEntities, endTokPad=numEntities;
      const spans=[];
      logits.forEach((value,id)=>{
        const startToken=Math.floor(id/startTokPad)%inputLength;
        const endToken=startToken+Math.floor(id/endTokPad)%MAXW;
        const entity=id%numEntities; const prob=sigmoid(value);
        if(prob>=threshold && startToken<textLength && endToken<textLength){
          spans.push([text.slice(ws[startToken],we[endToken]),ws[startToken],we[endToken],idToClass[entity+1],prob]);
        }
      });
      // greedy flat
    ...
    return {ok:true,
      ex1: await extract("Barack Obama was born in Honolulu Hawaii", ["person","location"]),
      ex2: await extract("Apple was founded by Steve Jobs in California in 1976", ["company","person","location","date"]),
      ex3: await extract("The Eiffel Tower is in Paris France", ["landmark","city","country"])};
   } catch(e){ return {ok:false, err:String(e&&e.stack||e&&e.message||e).slice(0,400)}; }
  })()`, 300000);

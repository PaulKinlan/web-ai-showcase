#!/usr/bin/env node
// Route-complete distilbert-base-uncased acceptance: real browser inference on every published route
// at desktop and mobile. Advertised stage driven for real:
//   Xenova/distilbert-base-uncased (all routes — fill-mask masked-LM, q8 WASM)
...
const RUN_RECORD = join(repoRoot, "models/distilbert-base-uncased/acceptance-run.json");
const PROFILE_DIR = mkdtempSync(join(tmpdir(), "distilbertfm-acceptance-"));
if (WRITE_RUN) rmSync(RUN_RECORD, { force: true });

const MODEL_ID = "Xenova/distilbert-base-uncased";
const ROUTES = {
  overview: "models/distilbert-base-uncased/",
  basics: "models/distilbert-base-uncased/basics/",
  practical: "models/distilbert-base-uncased/practical/",
  wild: "models/distilbert-base-uncased/wild/",
};

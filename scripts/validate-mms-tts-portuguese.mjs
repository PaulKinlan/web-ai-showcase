#!/usr/bin/env node
// Route-complete mms-tts-portuguese acceptance: real browser inference on every published route at
// desktop and mobile. Advertised stage driven for real:
//   Xenova/mms-tts-por  (all routes — MMS-TTS Portuguese VITS synthesis, WASM q8, ~38 MB)
...
const RUN_RECORD = join(repoRoot, "models/mms-tts-portuguese/acceptance-run.json");
const PROFILE_DIR = join(homedir(), ".cache", "webai-validator-profiles", "mms-tts-portuguese");
mkdirSync(PROFILE_DIR, { recursive: true });
if (WRITE_RUN) rmSync(RUN_RECORD, { force: true });

const STAGE = "Xenova/mms-tts-por"; // the one advertised model stage
const ROUTES = {
  overview: "models/mms-tts-portuguese/",
  basics: "models/mms-tts-portuguese/basics/",
  practical: "models/mms-tts-portuguese/practical/",
  wild: "models/mms-tts-portuguese/wild/",
};
...
async function exercise(cdp, page, rung, viewport) {
  ...
  const SAMPLE1 = "Esta voz é gerada inteiramente no seu navegador, sem nenhum servidor.";
  const SAMPLE2 = "Aprender português é uma viagem fascinante pela língua e pela cultura.";

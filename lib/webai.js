...
/** True if this browser exposes WebGPU. Model pages must degrade honestly when false. */
export function hasWebGPU() {
  return typeof navigator !== "undefined" && "gpu" in navigator;
}

/** Pick the device for a model: prefer its declared backend, fall back to wasm without WebGPU. */
...
export async function loadPipeline({ task, model, backend, dtype, onProgress }) {
  ...
  const device = pickDevice(backend);

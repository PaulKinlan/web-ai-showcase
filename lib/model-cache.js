 ...
 */
export async function inspectModel({ key, modelId, timeoutMs = 5000 }) {
  const [record, cachedNow] = await settleWithin(
    Promise.all([getRecord(key), scanCachedFiles(modelId)]),
    timeoutMs,
  ...
  const revisionTimeoutMs = Math.min(timeoutMs, 4000);
  const remote = await settleWithin(
    remoteRevision(modelId, revisionTimeoutMs),
    revisionTimeoutMs,

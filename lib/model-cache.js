...
/**
 * Fast path for a validated model: probe its recorded URLs against the caches.
 *
 * Single pass: enumerate each cache's request URLs ONCE into a Set, then test membership. The
 * previous implementation did N× `caches.match(url)`, and every `caches.match()` searches ALL
 * caches — for a multi-file model (e.g. a quantised LLM with several shards) on an origin with
 * several caches that was ~10-20 full cache scans per "Checking for a local copy…", which is why a
 * clearly-cached model could still take seconds to resolve.
 */
async function checkRecordedFiles(urls, timeoutMs) {
  ...
  const { present, missing } = await settleWithin(
    (async () => {
      const presentUrls = new Set();
      for (const name of await caches.keys()) {
        const cache = await caches.open(name);
        const keys = await cache.keys();
        for (let i = 0; i < keys.length; i++) presentUrls.add(keys[i].url);
      }
      const present = expected.filter((u) => presentUrls.has(u));
      const missing = expected.filter((u) => !presentUrls.has(u));
...
export async function inspectModel({ key, timeoutMs = 750 }) {
  ...
  const { present, missing } = await checkRecordedFiles(expected, timeoutMs).catch(() => ({
    // A slow/pathological CacheStorage probe must never hold a validated model on the
    // "Checking for a local copy…" step — the validation record IS the fast signal, and the
    // runtime (transformers.js) re-validates its own cache during load, so proceeding optimistic
    // is honest: a genuinely evicted cache just shows normal download progress.
    present: [],
    missing: [],
  }));
  if (missing.length > 0) {
...
/** Record that a model loaded successfully — capture its cached files + the requested/current revision. */
export async function recordValidated({ key, modelId, runtime, dtype, revision: pinnedRevision }) {
  const [files, revision] = await Promise.all([
    scanCachedFiles(modelId),
    pinnedRevision ? Promise.resolve(pinnedRevision) : remoteRevision(modelId),
  ]);
  await putRecord({
    key,
    modelId,

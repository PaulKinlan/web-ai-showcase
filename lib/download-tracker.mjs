...
/** @typedef {"checking"|"discovering"|"downloading"|"paused"|"verifying"|"initialising"|"ready"|"error"} Phase */
...
export function createDownloadTracker({ knownFiles } = {}) {
  ...
  // Runtime-owned aggregate: some runtimes (WebLLM, MediaPipe) don't expose per-file bytes — only an
  // overall fraction (WebLLM) or nothing (MediaPipe). We surface that HONESTLY as a single runtime-owned
  // aggregate (ratio 0..1 or null=indeterminate) rather than inventing fake per-file byte counts.
  let runtimeAggregate = null; // { ratio: number|null, label: string }
  ...
  function setPhaseFromState() {
    ...
    if (list.length === 0) {
      // Runtime-owned aggregate (WebLLM/MediaPipe): downloading until the fraction reaches 1, then the
      // runtime is still building the model (initialising) until it signals ready.
      if (runtimeAggregate) {
        phase = (runtimeAggregate.ratio != null && runtimeAggregate.ratio >= 1)
          ? "initialising"
          : "downloading";
      } else {
        phase = "checking";
    ...
    const anyPaused = list.some((f) => f.state === "paused");
    ...
    else if (anyPaused) phase = "paused"; // some transfer paused, partials kept — resume continues it
  ...
  function ingest(evt) {
    ...
    switch (status) {
      ...
      // Runtime-owned aggregate progress (WebLLM fraction, MediaPipe indeterminate). Honest: one overall
      // ratio, never fabricated per-file bytes.
      case "aggregate":
        runtimeAggregate = {
          ratio: typeof evt.ratio === "number" ? Math.min(1, Math.max(0, evt.ratio)) : null,
          label: evt.label || "model",
        };
        break;
      case "phase":
  ...
  function snapshot() {
    ...
    let indeterminate = anyUnknownTotal;
    ...
    // Runtime-owned aggregate mode (no per-file byte events): report the runtime's overall fraction.
    let runtimeOwned = false;
    let aggregateLabel = null;
    if (list.length === 0 && runtimeAggregate) {
      runtimeOwned = true;
      aggregateLabel = runtimeAggregate.label;
      ratio = ready ? 1 : runtimeAggregate.ratio; // ready ⇒ 100%
      indeterminate = ratio == null; // MediaPipe (no fraction) ⇒ honest indeterminate
    }
    return {
      phase,
      ready,
      ...
      aggregate: {
        ...
        complete,
        failed,
        runtimeOwned, // true ⇒ a library owns the transfer; ratio is the runtime's overall fraction, not our bytes
        runtimeLabel: aggregateLabel,
      },
    };
  }
...
export const PHASE_LABEL = {
  ...
  paused: "Paused — resume to continue where it left off.",

...
import { transformersAdapter, webllmAdapter } from "./download-adapters.mjs";
...
export function createModelLoader({ mount, model, load, onReady, onError }) {
  ...
  let tracker = null, tAdapter = null, wAdapter = null;
  const feed = (evt) => {
  ...
  function newLoadSession() {
    ...
    wAdapter = webllmAdapter(feed, { label: model.modelId });
    dl.reset();
  }
  const onProgress = (p) => {
    if (!p || typeof p !== "object") return;
    if (typeof p.status === "string") tAdapter?.(p); // TJS / MediaPipe (initiate/progress/done/ready)
    else if (typeof p.progress === "number") wAdapter?.(p); // WebLLM overall fraction
  };

...
export function createModelLoader({ mount, model, load, onReady, onError, dispose, onDispose }) {
  ...
  async function runLoad(kind) {
    ...
    try {
      ...
      try {
        ...
        validationWarning = "The browser did not save the local-cache validation record.";
      }
      setState("ready", validationWarning);
      addManageControls();
      onReady?.(instance);
  ...
  function addManageControls() {
    // Once ready, offer Release (frees RAM, keeps cache) when the demo supports it, plus cache-clear.
  ...
  async function start() {
    ...
    try {
      ...
      detailEl.textContent =
        `${model.modelId} · The browser's local storage/cache API did not respond. Continuing may download missing assets.`;
    ...
    switch (info.state) {
      ...
      case "update":
        ...
        // Append (don't replace) so the Release/Clear controls runLoad added stay available.
        await runLoad("init");
        ...
        actions.append(makeButton("Update to newer version", "secondary", updateModel));
        break;
      case "partial":

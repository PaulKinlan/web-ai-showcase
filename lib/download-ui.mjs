...
export function createDownloadUI({ mount, sizeMB, controls = {} }) {
  ...
  function paint(snap) {
    ...
    if (snap.phase === "ready") {
      aggEl.textContent = "";
    } else if (a.runtimeOwned) {
      // A library owns the transfer (WebLLM fraction / MediaPipe indeterminate): show the runtime's own
      // overall % (or an honest "size not reported"), never fabricated per-file byte counts.
      const label = a.runtimeLabel || "model";
      aggEl.textContent = a.ratio != null
        ? `${label} — ${Math.round(a.ratio * 100)}% (downloaded by the runtime)`
        : `${label} — downloading (size not reported by the runtime)`;
  ...
  return {
    ...
    async showStorage() {
      ...
      if (need) storageEl.textContent = need; // useful immediately; quota detail is optional.
      try {
        let timer;
      ...
      } catch { /* estimate unavailable or slow: never block model use */ }
    },

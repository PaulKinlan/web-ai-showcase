...
//
// Two independent pipelines, each with an honest fallback so they run on GitHub Pages (no COOP/COEP,
// and browsers that lack OffscreenCanvas-in-workers):
//
...
//        same-origin + COEP: require-corp, i.e. `self.crossOriginIsolated === true`. GitHub Pages
//        can't set those response headers, so SAB is UNAVAILABLE there.
//      • Therefore the DEFAULT path is postMessage: the worklet posts each bounded chunk to the main
 ...
 *   • "postmessage" (DEFAULT): the worklet posts each chunk to the main thread, which transfers it to
 *      the feature worker. Works with no special headers.
 */

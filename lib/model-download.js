...
/** Every persisted interruption-safe download on this origin, without touching the network. */
export async function listResumeStates() {
  let db;
  try {
    db = await openDB();
  } catch {
  ...
  const rows = await tx(db, META_STORE, "readonly", (t) => reqP(t.objectStore(META_STORE).getAll()))
    .catch(() => []);
  db.close();
  return (Array.isArray(rows) ? rows : []).map((meta) => ({
    url: meta.url,
    total: meta.total ?? null,
    etag: meta.etag ?? null,
    sha256: meta.sha256 ?? null,

...
export function createResumableLoader(
  ...
  function readyControls() {
    ...
    if (updateInfo) {
      async function updateModel() {
        ui.setActions([]);
        ui.setStorageMessage("Clearing the older cached version…");
        try {
          await settleWithin(
            clearModelCache(model.modelId, key),
            5000,
          ...
          updateInfo = null;
          await beginLoad("download");
        } catch (err) {
          feed({
            status: "error",
            message: "The browser did not finish clearing the older model version.",
          });
          ui.setStorageMessage(String(err?.message || err));
          ui.setActions([{ label: "Retry update", className: "secondary", onClick: updateModel }]);
        }
      }
      controls.push({ label: "Update model", className: "secondary", onClick: updateModel });
    }
    ui.setActions(controls);
  }
  ...
  function discoverUpdate(info) {
    ...
    void checkModelUpdate({ modelId: model.modelId, record: info.record }).then((available) => {
      ...
      ui.setStorageMessage(
        "Update available. Keep using this cached model or update when convenient.",
      );
      if (instance) readyControls();
    });
  }
  ...
  async function beginLoad(kind) {
    ...
    applyControls("download");
    try {
      // No automatic lifecycle memory capture here either: the native API answers only after a GC pass
      // (measured ~19s idle, ~60s with a model resident; web-ai-showcase-cgr), so this await could only
      // ever resolve by timeout — a guaranteed 1s stall on every load. Measurement is the visitor click.
      await engine.load({
        onEvent: feed,
        onPaused: () => {
          paused = true;
      ...
      // (Model-ready memory capture removed with the rest of the automatic sampling.)
    } catch (err) {
      loadingActive = false;
      controlMode = "";
  ...
  async function start() {
    ...
    try {
      ...
      ui.setStorageMessage(
        `${String(err?.message || err)} You can retry or continue without waiting.`,
      );
      ui.setActions([
    ...
    switch (info.state) {
      case "current":
        // Valid local copy → AUTO-initialise now. Remote revision discovery runs separately and can
        // only add an optional Update action; it never delays local use.
        discoverUpdate(info);
        beginLoad("init");
        break;
      case "partial":

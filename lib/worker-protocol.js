...
export class WorkerClient {
  ...
  request(method, payload, options = {}) {
    if (this.state === "terminated" || this.state === "error") {
      return Promise.reject(
        new Error(
          `WorkerClient ${this.name} is ${this.state}; create a new worker before retrying`,
        ),
      );
    }
  ...
  _onFatal(message) {
    ...
    // Reject everything still outstanding; the worker is no longer trustworthy. Fatal is terminal:
    // reject all future requests immediately and tear down this worker. A caller may retry only by
    // creating a fresh WorkerClient (normally through the model loader's release/reinitialise path).

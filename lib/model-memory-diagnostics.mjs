...
export class ModelMemoryDiagnostics extends HTMLElementBase {
  ...
  connectedCallback() {
    ...
    this.$("#measure").addEventListener(
      "click",
      () => this.capture("Manual snapshot", { timeoutMs: 15000 }),
    );
    const support = memorySupport();
    ...
    this.$("#explanation").textContent = support.supported
      ? "Browser estimate for this page and its dedicated workers, not model-only allocation, GPU memory, or disk cache. Deltas compare observations; garbage collection and other page activity affect them."
      : `${support.reason} Use the cross-origin-isolated Deno deployment in a supported Chromium browser.`;
  }

  async capture(label = "Snapshot", { timeoutMs = 1000 } = {}) {
    const support = memorySupport();
    ...
    this.measuring = measureOriginMemory().then((measurement) => {
      // A late measurement cannot truthfully be labelled "before load" or "before release".
      if (expired || !this.isConnected) return null;

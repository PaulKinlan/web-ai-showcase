// Front-end helpers shared by every DPT (Intel dpt-hybrid-midas) page. Keeps pages thin: owns the
// worker handshake, turns files/samples into data URLs, colourises the depth map, and does the
// per-pixel parallax warp. All inference lives in worker.js (off the main thread).
//
// The API and display are the same shape as Depth Anything — a normalized 0–255 depth image plus the
// raw predicted-depth tensor — but the model underneath is DIFFERENT: DPT is the original dense
// prediction transformer (ViT-hybrid encoder + MiDaS training), not a DINOv2 backbone.

const WORKER_URL = "/web-ai-showcase/models/dpt-depth/worker.js";

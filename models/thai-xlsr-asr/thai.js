// Front-end helpers for the wav2vec2-XLSR-53 (Thai) pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT from the English, Russian and other XLSR demos: this model is XLSR-53 fine-tuned for THAI —
// a Thai-script recogniser over a 70-symbol vocab (Thai consonants, vowels and tone marks) plus a "|"
// word-boundary symbol we render as a space (Thai has no native inter-word spaces). It is
// language-specialised for Thai. The engine is otherwise the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the Thai alphabet.

const WORKER_URL = "/web-ai-showcase/models/thai-xlsr-asr/worker.js";
const TARGET_RATE = 16000;

export class ThaiXlsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Italian word + start–end seconds). */

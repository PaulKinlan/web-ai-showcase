// CamemBERT fill-mask worker — inference off the main thread so the control UI stays responsive.
// Model: Xenova/camembert-base (task: fill-mask / masked language modelling), WASM, q8.
//
// CamemBERT is a RoBERTa trained ONLY on French (the 138 GB OSCAR French corpus), so its 32k-token
// SentencePiece vocabulary and its predictions are French-native — no language flag, no shared
// multilingual budget. Its mask token is "<mask>" (RoBERTa style), NOT "[MASK]".
//
...
// logits — the raw pre-softmax scores over the whole 32,005-token French vocabulary — not just the
// top-k a pipeline hands back. From the logits row at each <mask> we softmax ourselves.
//
// Operations:
...
//                     (compare P(word_A) vs P(word_B) at the mask across French templates).
...
// Trim it so the UI shows the clean French word.
...
// without sorting the whole 32k vocab: maintain a small ordered buffer (same approach as the BERT page).

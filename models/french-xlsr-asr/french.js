// Front-end helpers for the wav2vec2-XLSR-53 (French) pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT from the English wav2vec2 demo AND the Russian XLSR demo: this model is XLSR-53 fine-tuned for
// FRENCH — a Latin-script recogniser over a 59-symbol vocab with the full set of French diacritics — so
// it is language-specialised for French. The engine is otherwise the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the French alphabet.

const WORKER_URL = "/web-ai-showcase/models/french-xlsr-asr/worker.js";
const TARGET_RATE = 16000;

export class FrenchXlsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (French word + start–end seconds). */

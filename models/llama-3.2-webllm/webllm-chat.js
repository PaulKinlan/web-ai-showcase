// Front-end helpers shared by every Llama-3.2-3B (WebLLM) page: the worker handshake + streaming
// plumbing, the honest WebGPU probe/fallback, and the chat widget CSS. All real inference happens
...
//
// This is the BIGGER Llama-3.2 sibling of models/llama-3-2-1b-webllm — same shared architecture, its
// own worker (3B build), ~1.9 GB download vs ~880 MB. Distinct route + model, additive to the catalogue.

export { webGPUAdapterAvailable } from "/web-ai-showcase/lib/webllm.js";

const WORKER_URL = "/web-ai-showcase/models/llama-3.2-webllm/worker.js";
 ...
 *  the ~1.9 GB download so a visitor knows what running this 3B model for real would cost. */
export function webgpuFallbackHTML(probe) {
  ...
  return `
    <strong>Llama 3.2 3B runs on WebLLM, which needs WebGPU — and it isn't available in this browser.</strong>
    <p class="muted" style="margin:.4rem 0">${why}${
    probe.detail ? " (" + escapeHTML(probe.detail) + ")" : ""
  }
    WebLLM has <em>no WASM fallback</em>: a 3B model decoding token-by-token needs a real GPU, and the
    weights are a <strong>~1.9 GB</strong> download (cached after the first load) needing roughly 2.3 GB of
    GPU memory. So this page won't fake a reply. To run it for real:</p>
    <ul class="muted" style="margin:.2rem 0">
      ...
      <li>The q4f16_1 build is fastest on GPUs that report the <code>shader-f16</code> feature; on a GPU with under ~3 GB of free memory it may fail to allocate — the smaller <a href="/web-ai-showcase/models/llama-3-2-1b-webllm/">Llama 3.2 1B</a> page is a lighter alternative.</li>
    </ul>`;
}
...
.chip{font:inherit;font-size:.78rem;padding:.2rem .6rem;border-radius:999px;border:1px solid var(--border);background:var(--bg-raised);color:var(--color);cursor:pointer;min-block-size:32px}
.chip:hover{border-color:var(--accent)}
.chip[aria-pressed=true]{border-color:var(--accent);background:var(--bg-secondary)}
.chips{display:flex;flex-wrap:wrap;gap:.4rem}
.tag-runtime { border-color:var(--accent); color:var(--accent); }
`;

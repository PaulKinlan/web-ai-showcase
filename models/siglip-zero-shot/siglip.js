// Shared front-end helpers for the SigLIP pages. Keeps each page thin: it owns the worker handshake,
// turns files/samples into data URLs, and renders the independent-sigmoid bars. All inference lives in
// worker.js (off the main thread). The one idea to hold onto: SigLIP's per-label scores are INDEPENDENT
// sigmoids, so a bar can read 0.9 for two labels at once, or ~0 for every label ("none of these").

const WORKER_URL = "/web-ai-showcase/models/siglip-zero-shot/worker.js";

export class SiglipEngine {
  constructor() {
  ...
  /**
   * Classify an image against labels.
   * @returns {Promise<{labels, logits, sigmoid, softmax, cosines, fit, imgDims, txtDims, ms, device}>}
   */
 ...
 * Unlike a softmax chart these do NOT sum to 100% — each bar is its own yes/no probability. Any bar
 * over `threshold` is treated as a positive match and highlighted.
 */
export function renderSigmoidBars(container, items, { threshold = 0.5, showCosine = false } = {}) {
...
/** Shared inline styles for the SigLIP-specific widgets (bars, dropzone, threshold). Injected once. */
export const SIGLIP_CSS = `
.dropzone {

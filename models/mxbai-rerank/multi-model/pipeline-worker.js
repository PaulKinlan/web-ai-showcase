// Two-model retrieve-then-rerank worker — both models run off the main thread.
//   Stage 1 (retriever): Xenova/gte-small (feature-extraction) — a fast GTE bi-encoder for embeddings.
//   Stage 2 (reranker):  mixedbread-ai/mxbai-rerank-xsmall-v1 (DeBERTa-v2 cross-encoder) — precise
//     relevance on the shortlist.
// This is the classic production pattern: embeddings cast a wide net cheaply; the cross-encoder reads
// each shortlisted (query, passage) pair together and reorders for precision. We report BOTH orders so
// the value of stage 2 is visible.

import { loadPipeline } from "/web-ai-showcase/lib/webai.js";

const EMBED_ID = "Xenova/gte-small";
const RERANK_ID = "mixedbread-ai/mxbai-rerank-xsmall-v1";
...
async function ensureLoaded() {
  ...
  if (!embedPipe) {
    post({ type: "stage", stage: "Loading GTE retriever…" });
  ...
  if (!rerankPipe) {
    post({ type: "stage", stage: "Loading mxbai reranker…" });
...
async function retrieveThenRerank(id, query, passages, topK) {
  ...
  // --- Stage 1: embed query + passages, rank by cosine (the retriever) ---

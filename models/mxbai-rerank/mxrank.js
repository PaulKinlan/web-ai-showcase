// Front-end helpers for the mxbai-rerank cross-encoder pages. Owns the worker handshake and the
// renderers (ranked passage list, before/after reordering, decision-margin + lexical contrast).
// All inference lives in worker.js (off the main thread).
//
// Model: mixedbread-ai/mxbai-rerank-xsmall-v1 — a DeBERTa-v2 cross-encoder (num_labels=1). Unlike the
// BGE (XLM-RoBERTa) / MS-MARCO (BERT-MiniLM) / Jina (JinaBERT) rerankers already in the showcase, this
// one's backbone is DeBERTa-v2 with disentangled attention; it is English-focused and mixedbread's own
// rerank family. Same cross-encoder contract (read (query, passage) together → one relevance logit),
// different scoring behaviour — which is exactly what these pages surface.

const WORKER_URL = "/web-ai-showcase/models/mxbai-rerank/worker.js";
...
export class RerankEngine {
  ...
  /** Rerank → { query, results:[…original order], ranked:[…by logit], margin, ms, device }. */
 ...
 * logit and its sigmoid probability stay as numbers.
 */
...
/**
 * mxbai-flavoured "see inside": the DECISION MARGIN. A cross-encoder isn't just an ordering — the size
 * of the logit gap between #1 and the runners-up tells you how DECISIVELY the top passage won. A big
 * margin = a confident answer selection; a small margin = the top answers are close and you may want to
 * feed several to the LLM. We show every gap between adjacent ranks.
 */
export function renderMargins(container, ranked) {
  if (ranked.length < 2) {
    container.innerHTML = "";
    return;
  }
  const gaps = [];
  for (let i = 1; i < ranked.length; i++) gaps.push(ranked[i - 1].logit - ranked[i].logit);
  const maxGap = Math.max(...gaps, 0.0001);
  const shorten = (s) => (s.length > 40 ? s.slice(0, 39) + "…" : s);
  const rows = gaps.map((g, i) => {
    const w = (g / maxGap) * 100;
    const big = i === 0 && g === maxGap;
    return `<div class="mg-row ${big ? "lead" : ""}">` +
      `<span class="mg-lab">#${i + 1}→#${i + 2}</span>` +
      `<span class="mg-bar"><span class="mg-fill" style="inline-size:${
        w.toFixed(1)
      }%"></span></span>` +
      `<span class="mg-val">Δ ${g.toFixed(2)}</span></div>`;
  }).join("");
  container.innerHTML =
    `<div class="mg-top">Top pick: <b>${
      escapeHTML(shorten(ranked[0].passage))
    }</b> — wins the #1→#2 slot by <b>Δ ${gaps[0].toFixed(2)}</b> logits</div>` + rows;
}
...
.mg-wrap { display: flex; flex-direction: column; gap: .35rem; margin-top: .5rem; }
.mg-top { font-size: .84rem; margin-bottom: .3rem; }
.mg-row { display: grid; grid-template-columns: 5.5rem 1fr 4rem; gap: .5rem; align-items: center; }
.mg-row.lead .mg-fill { background: var(--accent); }
.mg-lab { font-family: var(--font-mono); font-size: .74rem; color: var(--muted); }
.mg-bar { block-size: .6rem; background: var(--bg-secondary); border: 1px solid var(--border); border-radius: 999px; overflow: hidden; }
.mg-fill { display: block; block-size: 100%; background: var(--border-strong); }
.mg-val { font-family: var(--font-mono); font-size: .74rem; color: var(--muted); text-align: right; }

// Front-end helpers shared by every BEN2 page. Owns the worker handshake, turns files/samples into
// data URLs, and composes the cutout (apply the alpha matte to the photo, over any backdrop). All
// inference AND the dense per-pixel composite live in worker.js (off the main thread) — the main thread
// only draws a returned ImageBitmap.

const WORKER_URL = "/web-ai-showcase/models/ben2-background-removal/worker.js";

export class Ben2Engine {
  constructor() {
...
/** On-demand image classifier (MobileNet v2) for the multi-model rung: matte → classify the subject.
 *  Own worker + protocol so BEN2 stays self-contained. */
export class ClassifyEngine {
  constructor() {
    this.worker = new Worker("/web-ai-showcase/models/ben2-background-removal/classify-worker.js", {
      type: "module",
    });
    this.ready = false;
    this.device = "wasm";
...
/**
 * Compose the subject cutout. Draws into `canvas`.
...
export function composeCutout(
  ...
  // Fallback (no bitmap, or a feather multiplier that re-weights the baked alpha).
 ...
 * `cutout` ImageBitmap is passed and no outline is requested, draw it directly; otherwise fall back to
 * the per-pixel path.
 */
...
/** Highlight only the soft edge (partial alpha) — shows how clean the matte's boundary is (BEN2's
 *  confidence-guided matting aims for crisp, detailed edges: hair, fur, wisps). */
...
.slow-note { border: 1px solid var(--border); border-radius: var(--radius); background: var(--bg-raised); padding: .6rem .8rem; font-size: .82rem; color: var(--muted); margin: .5rem 0; }
`;

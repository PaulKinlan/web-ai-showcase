// wav2vec2 (Romanian) CTC worker — ALL inference off the main thread.
//
// DISTINCT model + language from every other ASR demo in this showcase. This is a ROMANIAN speech
// recogniser: facebook/wav2vec2-base-10k-voxpopuli fine-tuned on Romanian, exported to ONNX by the
// OpenVoiceOS project. All the built ASR demos (English wav2vec2, Russian/French/Spanish/Italian/
// Portuguese/Chinese/Japanese/Korean/Thai XLSR + Finnish/Polish/Czech/Croatian VoxPopuli) are CTC too,
// but none is Romanian.
//
// HONEST NOTE ON THE RUNTIME. Like the Czech/Croatian VoxPopuli demos (and unlike the Polish/Finnish
// onnx-community mirrors that load straight through transformers.js as AutoModelForCTC), the ONLY browser
// ONNX export of the Romanian VoxPopuli checkpoint is OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-ro-onnx,
// which is packaged for the onnx-asr toolkit: its config.json declares `model_type: "wav2vec2-ctc"` (a type
// transformers.js does NOT register → "Unsupported model type: wav2vec2-ctc"), the graph takes
// `input_values` + `input_lengths` and emits `logprobs` (log-softmax) rather than `logits`, and feature
// normalisation is baked INTO the graph. So this demo runs the raw ONNX graph directly on ONNX Runtime Web
// (the same engine transformers.js uses under the hood), feeding raw 16 kHz audio and CTC-decoding the
// log-probs ourselves. It is a real, in-browser run of the genuine Romanian Meta-AI checkpoint — not a
// relabel of another language and not a fake.
//
// LAYOUT / dtype: single fp32 graph (`model.onnx` ~1.7 MB) + external weights (`model.onnx.data` ~360 MB).
// No quantised export exists yet (onnxruntime.quantization can't process this dynamo-exported graph — see
// the model card), so we honestly ship the fp32 export and disclose the ~360 MB download. Verified in real
// headless Chrome (ORT Web 1.20.1, WASM EP) to produce real log-probs [1, frames, 37] and a coherent
// Romanian transcript on a CC0 VoxPopuli sample. Weights are fetched from the canonical HF repo and cached
// in Cache Storage under the repo URL so the shared model-cache layer detects them (auto-init on return
// visits).
...
const MODEL_ID = "OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-ro-onnx";
const MODEL_BASE = `https://huggingface.co/${MODEL_ID}/resolve/main/`;
const VOCAB_URL = "/web-ai-showcase/models/romanian-voxpopuli-asr/vocab.txt";
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[wav2vec2-ro worker]", err);

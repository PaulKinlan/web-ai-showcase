// Front-end helpers for the Romanian wav2vec2 ASR pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT language from every other ASR demo in this showcase: this is ROMANIAN — wav2vec2-base-10k-
// voxpopuli fine-tuned on Romanian (OpenVoiceOS ONNX export), a Latin-script CTC recogniser over a
// 37-symbol vocab with the Romanian diacritics (ă â î ș ț). NOT the wav2vec2-large-
// XLSR-53 model (no Romanian XLSR-53 ONNX exists) — see the model page's at-a-glance. Because the only
// Romanian ONNX export is an onnx-asr graph transformers.js can't load (model_type "wav2vec2-ctc"), the
// worker runs it directly on ONNX Runtime Web. The engine is the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the Romanian alphabet.

const WORKER_URL = "/web-ai-showcase/models/romanian-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class RomanianAsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Romanian word + start–end seconds). */

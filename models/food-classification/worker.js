// Food-101 image-classification worker — runs ALL inference off the main thread so the control UI
// stays responsive. Like the ViT demo it runs the LOW-LEVEL forward pass (processor → model) instead
// of the convenience pipeline, so a single pass yields everything both surfaces need: the raw
// pre-softmax logits, the full softmax distribution over the 101 dishes, the human-readable dish
// names, and confidence diagnostics (entropy, top-1 margin).
//
// Model: onnx-community/swin-finetuned-food101-ONNX (task: image-classification), WASM backend, q8
// (model_quantized.onnx). A Swin Transformer fine-tuned on Food-101 — 101 dish classes (apple_pie,
// bibimbap, sushi, …). Swin builds hierarchical feature maps with shifted local-window attention, so
// it reads fine-grained food texture (crumb, char, garnish) that a plain global-attention ViT can miss.
...
async function run(id, imageURL, topK) {
  ...
  const logits = Array.from(output.logits.data); // [101] raw, pre-softmax

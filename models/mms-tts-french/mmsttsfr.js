// Front-end helpers for the MMS-TTS French (VITS) pages. Thin: owns the worker handshake, WAV encoding
// and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: Xenova/mms-tts-fra — the FRENCH VITS checkpoint from Meta's MMS family (ONNX q8, ~38 MB, 16 kHz
// mono output). This is a DISTINCT model from the built English/German/Spanish MMS-TTS demos: its own
// French character vocab + French-trained VITS weights, so it speaks French orthography (accents
// é/è/ê/à/ç, liaisons, nasal vowels) natively.

const WORKER_URL = "/web-ai-showcase/models/mms-tts-french/worker.js";
export const MODEL_ID = "Xenova/mms-tts-fra";

export class MmsFrEngine {
  constructor() {
  ...
  /** Synthesize French text → { audio: Float32Array, rate, ms, samples, device } */
...
// A small set of French example lines that exercise accents, liaisons and French phrasing.
export const FRENCH_SAMPLES = [
  {
    label: "Présentation",
    text: "Cette voix est générée entièrement dans votre navigateur, sans aucun serveur.",
  },
  {
    label: "Virelangue",
  ...
  {
    label: "Accents",
    text: "À Noël, la fée éblouie déchiffra un très étrange grimoire près de la cheminée.",
  },
  {
    label: "Annonce",
    text: "Votre rendez-vous est confirmé pour mardi à neuf heures et demie. Merci d'être ponctuel.",
  },
  {
    label: "Météo",
    text: "Ce week-end, le temps sera plutôt ensoleillé avec des maximales autour de vingt-deux degrés.",
  },
  {
    label: "Nasales",
    text: "Un bon vin blanc, un grand pont ancien et un jardin plein de pins parfument le printemps.",
  },
];

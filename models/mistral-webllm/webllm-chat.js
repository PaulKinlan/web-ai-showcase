// Front-end helpers shared by every Mistral-7B-Instruct (WebLLM) page: the worker handshake +
// streaming plumbing, the honest WebGPU gate/fallback, Mistral's system-prompt folding, and the chat
...
// task mandates.
//
// Mistral Instruct's template is [INST] … [/INST] with NO dedicated system role, so a "system prompt"
// is folded into the first user instruction — analogous to Gemma, via [INST] instead of turn tokens.

export { webGPUAdapterAvailable } from "/web-ai-showcase/lib/webllm.js";

const WORKER_URL = "/web-ai-showcase/models/mistral-webllm/worker.js";

export class MistralChatEngine {
  constructor() {
...
/**
 * Mistral Instruct has NO dedicated system role — the prompt is [INST] user [/INST] assistant. So a
 * "system prompt" is folded into the first user instruction. This returns a NEW messages array with
 * any system content prepended to the first user message, guaranteeing a valid Mistral conversation.
 * Display THIS (not the pre-fold array) as the wire, so "see inside" is honest about what the model
 * receives.
 */
export function foldSystemForMistral(messages) {
 ...
 *  the ~4 GB download so a visitor knows what running this heavy model for real would cost. */
export function webgpuFallbackHTML(probe) {
  ...
  return `
    <strong>Mistral 7B Instruct runs on WebLLM, which needs WebGPU — and it isn't available in this browser.</strong>
    <p class="muted" style="margin:.4rem 0">${why}${
    probe.detail ? " (" + escapeHTML(probe.detail) + ")" : ""
  }
    WebLLM has <em>no WASM fallback</em>: a 7B model decoding token-by-token needs a real GPU, and the
    weights are a <strong>~4 GB</strong> download (cached after the first load) needing roughly 4.5 GB of
    GPU memory. This is the heaviest model in the catalogue, so this page won't fake a reply. To run it
    for real:</p>
    <ul class="muted" style="margin:.2rem 0">
      <li>Open in <strong>Chrome or Edge 113+</strong> on a machine with a capable GPU (a desktop or a recent, well-specced laptop).</li>
      ...
      <li>The q4f16_1 build is fastest on GPUs that report the <code>shader-f16</code> feature; on a GPU with under ~5 GB of memory it may fail to allocate.</li>
    </ul>`;
}
...
.msg { display:flex; flex-direction:column; gap:.15rem; max-inline-size:92%; }
...
.composer textarea { flex:1 1 auto; field-sizing:content; width:auto; min-block-size:2.6lh; max-block-size:8lh; resize:vertical; }

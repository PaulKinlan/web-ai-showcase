// Front-end helpers for the HuBERT feature pages. Keeps each page thin: worker handshake, audio decode
...
//
// HuBERT emits per-frame 768-dim embeddings, not text. We mean-pool + L2-normalise a clip's frames into
// one utterance vector and compare vectors with cosine similarity — the basis of audio-audio
// similarity, "find a similar segment", and clustering by self-supervised features. HuBERT's twist vs
// WavLM: it was pretrained by MASKED CLUSTER PREDICTION (predict offline k-means cluster ids for masked
// frames), not masked-speech + denoising — so its features organise speech into phone-like units.

const WORKER_URL = "/web-ai-showcase/models/hubert-features/worker.js";
const TARGET_RATE = 16000;

export class HubertEngine {
  constructor() {
...
/** HuBERT widget styles, injected once per page (keeps us on the shared design system). */
export const HUBERT_CSS = `

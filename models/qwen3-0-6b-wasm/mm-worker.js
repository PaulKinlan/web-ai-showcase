// Qwen3 multi-model worker — a real two-model RAG composition, off the main thread:
//   1. all-MiniLM-L6-v2 (Transformers.js feature-extraction) embeds the question + every knowledge note
//      and picks the most similar note by cosine similarity (retrieval).
//   2. Qwen3-0.6B answers the question grounded ONLY in that retrieved note (generation).
// Embedder: Xenova/all-MiniLM-L6-v2 (~25 MB, q8). LLM: onnx-community/Qwen3-0.6B-ONNX (~618 MB, q8).
// Classic Retrieval-Augmented Generation, entirely on-device.
...
async function run(id, query, notes) {
  ...
  let text = Array.isArray(full) ? (full.at(-1)?.content ?? "") : String(full ?? "");

// MMS-TTS Odia (VITS) worker — end-to-end Odia speech synthesis off the main thread so the control UI
...
// is no separate vocoder: Odia text → language-specific token ids → (flows + duration) → 16 kHz
// waveform, all in one network. Nothing here is faked.
//
// This demo is the ODIA checkpoint (naklitechie/mms-tts-or-ONNX) — a materially DISTINCT model from the
// other built MMS-TTS demos: its own native Odia-script character vocabulary (76 symbols,
// is_uroman:false — the rounded Odia script, distinct from Devanagari, Tamil, Malayalam and Gurmukhi)
// and its own VITS weights, trained on Odia speech. Canonical language tag ory.
//
// PROVENANCE + TOKENISER NOTE (why this worker loads the model + tokenises directly instead of the
// pipeline): naklitechie/mms-tts-or-ONNX is a faithful ONNX export whose vocab.json is BYTE-IDENTICAL to
// the canonical facebook/mms-tts-ory, but the repo ships model.onnx at the repo root (not onnx/) and does
...
// requires. (In this checkpoint the pad/blank token content happens to be the letter "ଛ", which the vocab
// maps to id 0 — exactly as the canonical checkpoint does, so a literal "ଛ" and the blank share id 0;
// this is faithful, not a bug.) This manual tokeniser was VERIFIED byte-for-byte identical to
// transformers.js's real VitsTokenizer on a control language (Xenova/mms-tts-hin: realLen 45 == manualLen
// 45, identical id sequence), so the Odia tokenisation is faithful — not a guess. The VITS model itself
// runs via the real transformers.js AutoModel, so the waveform is genuine model output.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const DEFAULT_MODEL = "naklitechie/mms-tts-or-ONNX";
...
async function ensureLoaded(modelId, notifyProgress) {
  ...
  state.Tensor = Tensor;
  // Canonical vocab (identical to facebook/mms-tts-ory) for the deterministic character tokeniser.
...
async function synth(id, modelId, text) {
  ...
  if (inputIds.length <= 1) {
    post({ type: "error", id, message: "No Odia characters recognised in the input." });
    return;
  }

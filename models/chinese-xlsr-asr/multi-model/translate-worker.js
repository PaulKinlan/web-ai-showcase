// Chinese→English translation worker for the Mandarin XLSR multi-model demo. Runs OPUS-MT (MarianMT)
// off the main thread so the ASR→translate chain never blocks the UI.
// Model: Xenova/opus-mt-zh-en (task: translation). Same MarianMT class as the built marianmt-translation
// demo, Chinese→English language pair.
...
async function ensureLoaded() {
  ...
  translator = await pipeline("translation", MODEL_ID, {
    device: DEVICE,
    dtype: "q8", // onnx/*_quantized — verified to translate zh→en on WASM
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[opus-mt-zh worker]", err);

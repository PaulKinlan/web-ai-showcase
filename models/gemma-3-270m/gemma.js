// Front-end helpers for the Gemma 3 270M pages: the worker handshake, streaming plumbing, and the
// shared widget CSS. Inference lives in worker.js (Transformers.js text-generation). Every Gemma page
// imports this so the chat loop and "see inside" surface stay identical.
//
// Gemma 3 270M is Google's smallest Gemma 3 (2025) — a ~270M instruction-tuned chat model, the "runs
// anywhere" tier of the family. The ONNX build (onnx-community/gemma-3-270m-it-ONNX) has the
// `gemma3_text` architecture, whose class landed in Transformers.js 4.2.0, so the worker pins that
// version LOCALLY. The worker probes for a real WebGPU adapter and runs on WebGPU (dtype q4f16, ~200 MB)
// when present, else on the universal WebAssembly/CPU path (dtype fp32, ~1.05 GB — the only export whose
// ops all run on the WASM EP; the quantized builds need WebGPU's GatherBlockQuantized).

const WORKER_URL = "/web-ai-showcase/models/gemma-3-270m/worker.js";

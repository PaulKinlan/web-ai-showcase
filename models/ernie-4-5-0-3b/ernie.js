// Front-end helpers for the ERNIE-4.5-0.3B pages: the worker handshake, streaming plumbing, and the
// shared widget CSS. Inference lives in worker.js (Transformers.js text-generation). Every ERNIE page
// imports this so the chat loop and "see inside" surface stay identical.
//
// ERNIE-4.5-0.3B is Baidu's 2025 open family — a compact, strongly bilingual (English + Chinese)
// instruct LLM. The ONNX build (onnx-community/ERNIE-4.5-0.3B-ONNX) has the `ernie4_5` architecture,
// whose class landed in Transformers.js 4.2.0, so the worker pins that version LOCALLY. The worker
// probes for a real WebGPU adapter and runs on WebGPU (dtype q4f16) when present, else on the universal
// WebAssembly/CPU path (dtype q8) — the point of this ~0.36B model is that it runs on ANY device.

const WORKER_URL = "/web-ai-showcase/models/ernie-4-5-0-3b/worker.js";

export class ErnieEngine {
  constructor() {

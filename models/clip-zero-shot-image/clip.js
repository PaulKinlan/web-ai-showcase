// Shared front-end helpers for the CLIP pages. Keeps each page thin: it owns the worker handshake,
// turns files/samples into data URLs, and renders the similarity bars. All inference is in worker.js.

const WORKER_URL = "/web-ai-showcase/models/clip-zero-shot-image/worker.js";
...
/**
 * Render similarity bars into `container`. `items` = [{label, prob, cosine?}], already sorted or not.
...
/** Shared inline styles for the CLIP-specific widgets (bars, dropzone). Injected once per page. */
...
.field-row label { display: flex; flex-direction: column; gap: .25rem; font-size: .82rem; flex: 1 1 240px; }
.field-row input[type=text] { inline-size: 100%; }
.readout {

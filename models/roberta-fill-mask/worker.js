// RoBERTa fill-mask worker — inference off the main thread so the control UI stays responsive.
// Model: Xenova/roberta-base (task: fill-mask / masked language modelling), WASM, q8.
//
// RoBERTa (Liu et al., 2019) is the canonical "BERT done right" encoder: same architecture as
// BERT-base (12 layers, ~125M params) but trained 10x longer on 160 GB of English text with no
// next-sentence-prediction and a byte-level BPE vocabulary of 50,265 tokens. Its mask token is
// "<mask>" (NOT BERT's "[MASK]").
//
...
// logits — the raw pre-softmax scores over the whole 50,265-token vocabulary — not just the top-k
...
// Byte-level BPE: decoding a word-initial token yields a leading space (Ġ→space); trim it so the
// UI shows the clean English word.
function cleanToken(idx) {
  return tokenizer.decode([idx]).trim();
}

// Softmax over one logits row, returning the top-k indices with logit + probability. Partial top-k
// without sorting the whole 50k vocab: maintain a small ordered buffer (same approach as the BERT page).
...
// Leading-subword id for a candidate word (byte-level BPE word-initial token). Multi-token
// candidates are compared by their leading token — a deliberate, disclosed simplification.

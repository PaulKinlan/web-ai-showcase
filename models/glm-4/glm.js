// Shared front-end helpers for the GLM-Edge-1.5B pages: the worker handshake, streaming plumbing,
// a real WebGPU probe (for the honest fallback), and the widget CSS. Inference lives in worker.js.
// GLM-Edge is Zhipu's bilingual (EN/ZH) GLM-4-architecture model in a browser-feasible 1.5B size.

const WORKER_URL = "/web-ai-showcase/models/glm-4/worker.js";
...
export class GLMEngine {
  constructor() {
    ...
    this._loadWaiters = [];
    this._probeWaiters = [];
    this._active = null; // { id, onToken, onPrompt, onFirst, resolve, reject }
    this._id = 0;
  ...
  _onMessage(msg) {
    switch (msg.type) {
      ...
      case "prompt":
        ...
        break;
      case "first":
        if (this._active && this._active.id === msg.id) this._active.onFirst?.(msg.t);
        break;
      case "token":
      ...
      case "done":
        if (this._active && this._active.id === msg.id) {
          this._active.resolve({ ms: msg.ms, tokens: msg.tokens, ttft: msg.ttft, text: msg.text });
          this._active = null;
        }
        break;
      case "topk-result":
        if (this._active && this._active.id === msg.id) {
          this._active.resolve({ prompt: msg.prompt, tokens: msg.tokens, vocab: msg.vocab });
          this._active = null;
        }
        break;
      case "error":
  ...
  /** Stream a chat completion. onToken(token, tMs) per token; onPrompt(template) + onFirst(t) once. */
  chat(messages, { onToken, onPrompt, onFirst, ...opts } = {}) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
      this._active = { id, onToken, onPrompt, onFirst, resolve, reject };
...
/**
 * The labelled needs-WebGPU state. GLM-Edge-1.5B is heavy (~1 GB) — WebGPU is the practical path — but
 * it CAN run on WASM (much slower, ~1.3 GB q4). We surface the real reason, the enable steps, AND an
 * honest opt-in WASM button. We never fake tokens.
 */
export function webgpuFallbackHTML(probe) {
  ...
  return `
    ...
    GLM-Edge-1.5B runs on WebGPU with <code>q4f16</code> (~1 GB). For real acceleration:</p>
    ...
    <p class="muted" style="margin:.4rem 0 0">This model can also run on the CPU via WebAssembly — a
    heavier ~1.3 GB <code>q4</code> download and much slower, but real. You can opt in below; nothing is
    faked either way.</p>`;
}

export const GLM_CSS = `
...
.bubble .caret { display:inline-block; inline-size:.5rem; block-size:1rem; background:var(--accent);
  vertical-align:text-bottom; animation:gblink 1s steps(2) infinite; }
...
.composer textarea { flex:1 1 12rem; resize:vertical; min-block-size:2.6rem; }
...
.chip { font:inherit; font-size:.78rem; padding:.28rem .6rem; border-radius:999px; min-block-size:2rem;

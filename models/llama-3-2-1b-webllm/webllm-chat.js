// Front-end helpers shared by every Llama-3.2-1B (WebLLM) page: the worker handshake + streaming
...
const WORKER_URL = "/web-ai-showcase/models/llama-3-2-1b-webllm/worker.js";
 ...
 *  the ~880 MB download so a visitor knows what running it for real would cost. */
export function webgpuFallbackHTML(probe) {
  ...
  return `
    <strong>Llama 3.2 1B runs on WebLLM, which needs WebGPU — and it isn't available in this browser.</strong>
    ...
    weights are a <strong>~880 MB</strong> download (cached after the first load). So this page won't

...
//   Stage 2 (reranker):  jinaai/jina-reranker-v2-base-multilingual (cross-encoder) — precise, multilingual.
// The classic production pattern, kept multilingual end to end: E5 embeddings cast a wide net across
// languages cheaply; the jina-v2 cross-encoder reads each shortlisted (query, passage) pair together and
...
//
// The reranker's ONNX config omits `model_type`, so we load AutoConfig, set model_type = "xlm-roberta"
// (the real backbone), and load AutoModelForSequenceClassification — no remote code, canonical weights.

import { loadPipeline, TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const EMBED_ID = "Xenova/multilingual-e5-small";
const RERANK_ID = "jinaai/jina-reranker-v2-base-multilingual";
...
async function ensureLoaded() {
  ...
  if (!rerankModel) {
    post({ type: "stage", stage: "Loading jina-reranker-v2…" });
    ...
    const config = await AutoConfig.from_pretrained(RERANK_ID, { progress_callback });

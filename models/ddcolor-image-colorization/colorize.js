// Front-end helpers shared by every DDColor colorization page. Keeps pages thin: owns the worker
// handshake (transferring ImageBitmaps so nothing is copied), turns files / gallery images into
// ImageBitmaps, and renders the colorized result + an accessible before/after reveal slider. ALL
// inference AND the dense Lab→RGB composite live in worker.js (off the main thread, raw ONNX Runtime
// Web). Privacy by construction: the image and every pixel it produces never leave the device.
//
// DDColor predicts CHROMA only — the luminance (all the detail) is the source image's, so the demo
// keeps the original structure exactly and only invents colour.

const WORKER_URL = "/web-ai-showcase/models/ddcolor-image-colorization/worker.js";

export class ColorizeEngine {
  constructor() {
  ...
  /** Colorize an ImageBitmap (transferred → zero-copy). Returns
   * { colorBmp, grayBmp, chromaBmp, w, h, imgW, imgH, ms, infMs, aStd, bStd, device }. */
  colorize(bitmap, opts) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
...
/** Draw an ImageBitmap into a canvas sized to fit within maxW while keeping aspect. */
...
// An accessible before/after reveal: two layered canvases (grayscale under, colour over, clipped by a
// draggable divider). The divider is a real <input type=range> so it is keyboard-operable, labelled,
// and announces its position — a native control, not a mouse-only widget.
export class RevealCompare {
  /** @param root a .reveal element containing .reveal-before (canvas), .reveal-after (canvas),
   *  .reveal-range (input[type=range]) and .reveal-handle. */
  constructor(root) {
    this.root = root;
  ...
  _apply() {
    ...
    // colour layer revealed from the LEFT up to pct
    this.after.style.clipPath = `inset(0 ${100 - pct}% 0 0)`;
    this.handle.style.insetInlineStart = pct + "%";
    this.range.setAttribute("aria-valuetext", `${Math.round(pct)}% colourised`);
  }
  /** Paint the grayscale (before) and colour (after) bitmaps, sizing both canvases identically. */
  set(grayBmp, colorBmp, maxW = 640) {
    drawBitmapFit(this.before, grayBmp, maxW);
    drawBitmapFit(this.after, colorBmp, maxW);
    this.after.width = this.before.width; // keep identical box
    this.after.height = this.before.height;
    const ctx = this.after.getContext("2d");
    ctx.drawImage(colorBmp, 0, 0, this.after.width, this.after.height);
    this._apply();
  }
}

/** Shared inline styles for the colorization widgets. Injected once per page. Structural only — all
 * colours come from the design-system CSS variables so light/dark theme for free. */
export const COLORIZE_CSS = `
...
.chroma-canvas { inline-size: 100%; max-inline-size: 300px; block-size: auto; border-radius: 6px; border: 1px solid var(--border); display: block; }
/* before/after reveal */
.reveal { position: relative; inline-size: 100%; border-radius: 6px; overflow: hidden;
  background: var(--bg-raised); touch-action: pan-y; }
...
.reveal-labels { display: flex; justify-content: space-between; font-family: var(--font-mono);

// Speech Commands (keyword spotting) worker — ALL inference off the main thread.
// The main thread decodes/records audio into a 16 kHz mono Float32Array and transfers it here; the
// worker runs the audio-classification pipeline and returns the top command labels with scores, the
...
//
// Model: Xenova/ast-finetuned-speech-commands-v2 (task: audio-classification), q8 ONNX, WASM (WebGPU
// when a real adapter exists). This is the SAME Audio-Spectrogram-Transformer architecture as the
// AudioSet demo, fine-tuned on Google Speech Commands v2 to recognise 35 short spoken keywords
// (yes/no/up/down/left/right/on/off/stop/go, the digits zero–nine, and more) instead of 527 general
// AudioSet sound classes. We import the SHARED loader from lib/webai.js — no invented API.

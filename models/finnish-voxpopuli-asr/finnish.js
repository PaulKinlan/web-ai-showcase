// Front-end helpers for the Finnish VoxPopuli-Uralic wav2vec2 pages. Keeps each page thin: worker
// handshake, audio decode to the 16 kHz mono Float32Array the model wants, a tiny mic recorder, waveform
// draw, the per-frame CTC strip renderer, and shared CSS. All inference lives in worker.js (off-thread).
//
// This model is wav2vec2-large pre-trained on the VoxPopuli Uralic subset and fine-tuned for Finnish
// (Finnish-NLP/wav2vec2-large-uralic-voxpopuli-v2-finnish) — NOT the XLSR-53 checkpoint (no Finnish
// XLSR-53 ONNX exists). A Latin-script recogniser over a 34-symbol vocab (a-z + ä å ö). The engine is
// the same CTC shape (per-frame argmax → collapse), which lets the page visualise the collapse.

const WORKER_URL = "/web-ai-showcase/models/finnish-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class FinnishVoxEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Finnish word + start–end seconds). */

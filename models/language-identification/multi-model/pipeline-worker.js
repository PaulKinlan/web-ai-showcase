...
//   Stage 1 (detector): onnx-community/xlm-roberta-base-language-detection-ONNX (text-classification).
...
const DETECT_ID = "onnx-community/xlm-roberta-base-language-detection-ONNX";

// Marian bilingual models that translate INTO English. Only these routes have a bundled translator.
const TRANSLATORS = {
...
async function detectAndTranslate(id, text) {
  ...
  const scores = await detectPipe(text, { top_k: 20 });
  const top = scores[0];

// MMS-TTS Bengali (VITS) worker — end-to-end Bengali speech synthesis off the main thread so the control UI
...
// is no separate vocoder: Bengali text → language-specific token ids → (flows + duration) → 16 kHz
// waveform, all in one network. Nothing here is faked.
//
// This demo is the BENGALI checkpoint (naklitechie/mms-tts-bn-ONNX) — a materially DISTINCT model from the
// built English/German/Spanish/French/Arabic/Vietnamese/Hindi MMS-TTS demos: its own native Bengali-script
// character vocabulary (58 symbols, is_uroman:false — native Bengali, NOT romanised Latin, and a different
// script from Hindi's Devanagari) and its own VITS weights, trained on Bengali speech.
//
// PROVENANCE + TOKENISER NOTE (why this worker loads the model + tokenises directly instead of the
// pipeline): naklitechie/mms-tts-bn-ONNX is a faithful ONNX export whose vocab.json is BYTE-IDENTICAL to
// the canonical facebook/mms-tts-ben, but the repo ships model.onnx at the repo root (not onnx/) and does
...
// VitsTokenizer on a control language (Xenova/mms-tts-hin: realLen 37 == manualLen 37, identical id
// sequence), so the Bengali tokenisation is faithful — not a guess. The VITS model itself runs via the real
// transformers.js AutoModel, so the waveform is genuine model output.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const DEFAULT_MODEL = "naklitechie/mms-tts-bn-ONNX";
...
async function ensureLoaded(modelId, notifyProgress) {
  ...
  state.Tensor = Tensor;
  // Canonical vocab (identical to facebook/mms-tts-ben) for the deterministic character tokeniser.
...
async function synth(id, modelId, text) {
  ...
  if (inputIds.length <= 1) {
    post({ type: "error", id, message: "No Bengali characters recognised in the input." });
    return;
  }

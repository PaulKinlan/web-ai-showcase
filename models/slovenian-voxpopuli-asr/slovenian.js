// Front-end helpers for the Slovenian wav2vec2 ASR pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT language from every other ASR demo in this showcase: this is SLOVENIAN — wav2vec2-base-10k-
// voxpopuli fine-tuned on Slovenian (OpenVoiceOS ONNX export), a Latin-script CTC recogniser over a
// 34-symbol vocab with the Slovenian diacritics (č š ž). NOT the wav2vec2-large-
// XLSR-53 model (no Slovenian XLSR-53 ONNX exists) — see the model page's at-a-glance. Because the only
// Slovenian ONNX export is an onnx-asr graph transformers.js can't load (model_type "wav2vec2-ctc"), the
// worker runs it directly on ONNX Runtime Web. The engine is the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the Slovenian alphabet.

const WORKER_URL = "/web-ai-showcase/models/slovenian-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class SlovenianAsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Slovenian word + start–end seconds). */
...
/** ASR widget styles, injected once per page (keeps us on the shared design system). */
export const SL_ASR_CSS = `

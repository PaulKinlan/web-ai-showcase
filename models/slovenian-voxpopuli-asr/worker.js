// wav2vec2 (Slovenian) CTC worker — ALL inference off the main thread.
//
// DISTINCT model + language from every other ASR demo in this showcase. This is a SLOVENIAN speech
// recogniser: facebook/wav2vec2-base-10k-voxpopuli fine-tuned on Slovenian, exported to ONNX by the
...
// VoxPopuli) are CTC too, but none is Slovenian.
//
...
// AutoModelForCTC), the ONLY browser ONNX export of the Slovenian VoxPopuli checkpoint is
// OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-sl-onnx, packaged for the onnx-asr toolkit: its
...
// in-browser run of the genuine Slovenian Meta-AI checkpoint — not a relabel of another language, not a fake.
//
...
// headless Chrome (ORT Web 1.20.1, WASM EP) to produce real log-probs [1, frames, 34] and a coherent
// Slovenian transcript on a CC0 VoxPopuli sample. Weights are fetched from the canonical HF repo and cached in
...
const MODEL_ID = "OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-sl-onnx";
const MODEL_BASE = `https://huggingface.co/${MODEL_ID}/resolve/main/`;
const VOCAB_URL = "/web-ai-showcase/models/slovenian-voxpopuli-asr/vocab.txt";
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[wav2vec2-sl worker]", err);

// MMS-TTS Tamil (VITS) worker — end-to-end Tamil speech synthesis off the main thread so the control UI
...
// is no separate vocoder: Tamil text → language-specific token ids → (flows + duration) → 16 kHz
// waveform, all in one network. Nothing here is faked.
//
// This demo is the TAMIL checkpoint (naklitechie/mms-tts-ta-ONNX) — a materially DISTINCT model from the
// built English/German/Spanish/French/Arabic/Vietnamese/Hindi MMS-TTS demos: its own native Tamil-script
// character vocabulary (58 symbols, is_uroman:false — native Tamil, NOT romanised Latin, and a different
// script from Hindi's Devanagari) and its own VITS weights, trained on Tamil speech.
//
// PROVENANCE + TOKENISER NOTE (why this worker loads the model + tokenises directly instead of the
// pipeline): naklitechie/mms-tts-ta-ONNX is a faithful ONNX export whose vocab.json is BYTE-IDENTICAL to
// the canonical facebook/mms-tts-tam, but the repo ships model.onnx at the repo root (not onnx/) and does
...
// sequence), so the Tamil tokenisation is faithful — not a guess. The VITS model itself runs via the real
// transformers.js AutoModel, so the waveform is genuine model output.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const DEFAULT_MODEL = "naklitechie/mms-tts-ta-ONNX";
...
async function ensureLoaded(modelId, notifyProgress) {
  ...
  state.Tensor = Tensor;
  // Canonical vocab (identical to facebook/mms-tts-tam) for the deterministic character tokeniser.
...
async function synth(id, modelId, text) {
  ...
  if (inputIds.length <= 1) {
    post({ type: "error", id, message: "No Tamil characters recognised in the input." });
    return;
  }

// Front-end helpers shared by every StableLM-2-Zephyr (WebLLM) page: the worker handshake + streaming
...
//
// Unlike Gemma 2, StableLM 2 Zephyr has a real system role in its chat template, so pages pass a
// { role: "system" } message straight through — no folding. buildMessages() just assembles the array.

export { webGPUAdapterAvailable } from "/web-ai-showcase/lib/webllm.js";

const WORKER_URL = "/web-ai-showcase/models/stablelm-webllm/worker.js";

export class StableLMChatEngine {
  constructor() {
...
/**
 * StableLM 2 Zephyr HAS a system role — so a system prompt is a real system message, not folded.
 * This returns a fresh messages array: an optional system message, prior history, then the new user
 * turn. Display THIS as the wire so "see inside" is honest about exactly what the model receives.
 */
export function buildMessages({ system, history = [], user }) {
  const msgs = [];
  const sys = (system || "").trim();
  if (sys) msgs.push({ role: "system", content: sys });
  for (const m of history) msgs.push(m);
  if (user != null) msgs.push({ role: "user", content: user });
  return msgs;
}
 ...
 *  the ~1 GB download so a visitor knows what running it for real would cost. */
export function webgpuFallbackHTML(probe) {
  ...
  return `
    <strong>StableLM 2 Zephyr runs on WebLLM, which needs WebGPU — and it isn't available in this browser.</strong>
    <p class="muted" style="margin:.4rem 0">${why}${
    probe.detail ? " (" + escapeHTML(probe.detail) + ")" : ""
  }
    WebLLM has <em>no WASM fallback</em>: a 1.6B model decoding token-by-token needs a real GPU, and the
    weights are a <strong>~1 GB</strong> download (cached after the first load). So this page won't

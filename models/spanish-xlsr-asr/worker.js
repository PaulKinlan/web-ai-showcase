// wav2vec2-XLSR-53 (Spanish) CTC worker — ALL inference off the main thread.
//
// DISTINCT model from the built English wav2vec2 (models/wav2vec2-asr), the Russian XLSR
// (models/xlsr-multilingual-asr) AND the French XLSR (models/french-xlsr-asr). All are CTC, but this one
// is wav2vec2-large-XLSR-53 fine-tuned on SPANISH (jonatasgrosman/wav2vec2-large-xlsr-53-spanish): the
// cross-lingual model pre-trained on 53 languages (56k hours) then specialised for Spanish. Its output
// alphabet is the 41-symbol Latin-Spanish vocab — a-z plus the Spanish diacritics (á é í ó ú ü) and the
// letter ñ. Feeding it English or French produces Spanish-phonetic gibberish; it is language-specialised
// for Spanish. That Spanish specialisation — and the XLSR-53 transfer that made a good Spanish recogniser
// trainable — is the point.
//
// Model: FinDIT-Studio/wav2vec2-large-xlsr-53-spanish-onnx (ONNX export of jonatasgrosman's Spanish
...
//     config from the ORIGINAL repo it was exported from (jonatasgrosman/wav2vec2-large-xlsr-53-spanish —
//     model_type "wav2vec2", Wav2Vec2ForCTC, vocab_size 41) and pass it in as `config`;
...
//   • we do the greedy CTC decode ourselves from the model's 41-symbol vocab, which also lets us surface
...
const MODEL = "FinDIT-Studio/wav2vec2-large-xlsr-53-spanish-onnx";
const BASE_MODEL = "jonatasgrosman/wav2vec2-large-xlsr-53-spanish"; // architecture config only (no weights)
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[xlsr-es worker]", err);

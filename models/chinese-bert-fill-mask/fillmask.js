// Front-end helpers for the Chinese BERT fill-mask pages. Thin: owns the worker handshake + renderers.
// All inference (masked-LM logits, softmax, candidate scoring) lives in worker.js, off the main thread.

const WORKER_URL = "/web-ai-showcase/models/chinese-bert-fill-mask/worker.js";
export const MASK = "[MASK]";

export class ChineseFillMaskEngine {
  constructor() {
...
/** The "see inside" character strip: every input token as a chip, so a visitor can SEE that Chinese
 *  BERT tokenises one Hànzì at a time (no word segmentation). Special/mask tokens are marked. */
...
.mask-chip { font: inherit; font-size: .82rem; padding: .2rem .6rem; border-radius: 999px;
...
.cand-row { display: grid; grid-template-columns: 6rem 1fr 3.4rem; align-items: center; gap: .5rem; margin: .18rem 0; }

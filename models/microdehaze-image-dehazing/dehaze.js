// Front-end helpers shared by every MicroDehazeNet page. Keeps pages thin: owns the worker handshake
// (transferring ImageBitmaps so nothing is copied), turns files / gallery images into ImageBitmaps, can
// synthesise a HAZY input (physical atmospheric-scattering model) so the demo can show the network
// clearing a known haze, and renders the dehazed result + an accessible before/after reveal slider. ALL
// inference AND the dense output composite live in worker.js (off the main thread, raw ONNX Runtime Web).
// Privacy by construction: the image and every dehazed pixel never leave the device.

const WORKER_URL = "/web-ai-showcase/models/microdehaze-image-dehazing/worker.js";
...
export class DehazeEngine {
  ...
  /** Dehaze an ImageBitmap (transferred → zero-copy). Returns
   * { dehazedBmp, hazeBmp, w, h, imgW, imgH, inContrast, outContrast, ms, infMs, device }. */
  dehaze(bitmap, opts) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
...
// Synthesise a HAZY input with the physical atmospheric-scattering model I = J·t + A·(1−t): the clean
// scene J is blended toward a bright airlight A by transmission t (lower t = thicker haze). `amount` is
// haze strength 0..1 mapped to t = 1 − 0.75·amount. A depth-like gradient makes the top (sky/distance)
// hazier than the bottom, like real fog. Honest by design — the user chooses the haze and watches the
// network remove it; genuinely hazy photos can be uploaded instead.
const WORK = 640;
export async function addHaze(bitmap, { amount = 0.7, airlight = 235, depthGradient = true } = {}) {
  ...
  const d = img.data;
  const base = 1 - 0.75 * amount; // transmission at the near/bottom of the frame
  for (let y = 0; y < h; y++) {
    // more haze toward the top (distance), less at the bottom (foreground)
    const depth = depthGradient ? (1 - 0.5 * (y / h)) : 1;
    const t = Math.max(0.08, base * depth);
    for (let x = 0; x < w; x++) {
      const i = (y * w + x) * 4;
      d[i] = d[i] * t + airlight * (1 - t);
      d[i + 1] = d[i + 1] * t + airlight * (1 - t);
      d[i + 2] = d[i + 2] * t + airlight * (1 - t);
    }
  }
...
export class RevealCompare {
  ...
  _apply() {
    ...
    this.range.setAttribute("aria-valuetext", `${Math.round(pct)}% dehazed`);
  }
...
/** Shared inline styles for the dehazing widgets. Structural only — colours from design-system vars. */
export const DEHAZE_CSS = `
...
.haze-canvas { inline-size: 100%; max-inline-size: 300px; block-size: auto; border-radius: 6px; border: 1px solid var(--border); display: block; }

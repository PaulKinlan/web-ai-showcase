// Front-end helpers for the MMS-TTS Yoruba (VITS) pages. Thin: owns the worker handshake, WAV encoding
// and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: Xenova/mms-tts-yor — the YORUBA VITS checkpoint from Meta's MMS family (ONNX q8, ~38 MB, 16 kHz
// mono output). This is a DISTINCT model from the built MMS-TTS demos: its own native Yoruba vocabulary
// (43 symbols, is_uroman:false — the Yoruba orthography is Latin-based but carries the sub-dot letters
// ẹ/ọ/ṣ and the tone marks à/á/è/é that mark Yoruba's three tones, which no other checkpoint's vocab
// includes) + Yoruba-trained VITS weights. Yoruba is a major West African tonal language. This Xenova
// export ships the full tokenizer (tokenizer.json), so it loads with the plain text-to-speech pipeline().

const WORKER_URL = "/web-ai-showcase/models/mms-tts-yoruba/worker.js";
export const MODEL_ID = "Xenova/mms-tts-yor";

export class MmsYoEngine {
  constructor() {
  ...
  /** Synthesize Yoruba text → { audio: Float32Array, rate, ms, samples, device } */
...
// A small set of Yoruba example lines that exercise the sub-dot letters (ẹ/ọ/ṣ) and the tone marks.
export const YORUBA_SAMPLES = [
  {
    label: "Ìfáàrà",
    text: "Ohùn yìí ni a ṣẹ̀dá pátápátá nínú ẹ̀rọ aṣàwákiri rẹ, láìsí olùpèsè kankan.",
  },
  {
    label: "Ìkíni",
    text: "Ẹ nlẹ o! Báwo ni ẹ ṣe wà? Inú mi dùn láti pàdé yín.",
  },
  {
    label: "Ojú ọjọ́",
    text: "Ní ìparí ọ̀sẹ̀ yìí, ojú ọjọ́ yóò mọ́ jù, ooru yóò sì pọ̀.",
  },
  {
    label: "Ìròyìn",
    text: "Àwọn olùwádìí ti ṣàfihàn ọ̀nà tuntun láti ṣàmúlò àwòṣe èdè pátápátá nínú ẹ̀rọ aṣàwákiri.",
  },
  {
    label: "Ìkéde",
    text: "Ẹ jọ̀wọ́ fetí sílẹ̀: ọkọ̀ ojú irin tí ń lọ sí Èkó yóò kúrò ní pẹpẹ karùn-ún lónìí.",
  },
  {
    label: "Òwe",
    text: "Ìmọ̀ tí a bá pín ń dàgbà; ìrìn àjò ẹ̀kọ́ kì í parí.",
  },
];

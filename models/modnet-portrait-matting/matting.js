// Front-end helpers shared by every MODNet matting page. Keeps each page thin: it owns the worker
// handshake, turns files/samples into data URLs, composes the cutout (stamp the alpha matte into the
// photo), builds an accessible before/after comparison slider, and renders the "see inside" views
// (grayscale matte + soft-edge highlight). ALL inference lives in worker.js (off the main thread).

const WORKER_URL = "/web-ai-showcase/models/modnet-portrait-matting/worker.js";
...
/** Draw a light/dark checkerboard (transparency indicator) into a canvas context. */
...
export function cutoutToPNG(img, w, h, alpha, outline = 0, cutout = null) {
  ...
  return outline > 0 ? addOutline(c, alpha, w, h, outline) : c;
}

/** Dilate the thresholded matte and paint a solid halo behind the subject — the classic sticker edge. */
...
/** Highlight only the soft edge (partial alpha) — shows how much fine hair/fur detail the matte holds. */
...
/**
 * Wire an accessible before/after comparison: a range input drives a clip on the "after" layer so the
 * user drags between the original photo and the composited result. The slider is a real <input
 * type=range> (keyboard + SR operable); `onMove` receives 0–100.
 */
export function wireCompare(rangeEl, afterEl, initial = 55) {
  const apply = (v) => {
...
.seg button { border: 0; border-radius: 0; background: var(--bg-raised); color: var(--color); font-size: .82rem; padding: .45rem .7rem; }
...
.slider-row input[type=range] { flex: 1 1 180px; min-inline-size: 0; accent-color: var(--accent); block-size: 28px; }
.slider-row output { font-family: var(--font-mono); font-size: .82rem; min-inline-size: 3ch; }
.compare { position: relative; inline-size: 100%; border-radius: 8px; overflow: hidden; border: 1px solid var(--border); background: var(--bg-raised); }
.compare .layer { display: block; inline-size: 100%; block-size: auto; }
.compare .after { position: absolute; inset: 0; inline-size: 100%; block-size: 100%; }
.compare .after canvas, .compare .before canvas { inline-size: 100%; block-size: auto; display: block; }

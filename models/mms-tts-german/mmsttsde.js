// Front-end helpers for the MMS-TTS German (VITS) pages. Thin: owns the worker handshake, WAV encoding
// and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: Xenova/mms-tts-deu — the GERMAN VITS checkpoint from Meta's MMS family (ONNX q8, ~38 MB, 16 kHz
// mono output). This is a DISTINCT model from the built English MMS-TTS demo: German character vocab +
// German-trained VITS weights, so it speaks German orthography (ä/ö/ü/ß) natively.

const WORKER_URL = "/web-ai-showcase/models/mms-tts-german/worker.js";
export const MODEL_ID = "Xenova/mms-tts-deu";

export class MmsDeEngine {
  constructor() {
  ...
  /** Synthesize German text → { audio: Float32Array, rate, ms, samples, device } */
...
// A small set of German example lines that exercise umlauts, ß and German phrasing.
export const GERMAN_SAMPLES = [
  {
    label: "Vorstellung",
    text: "Diese Stimme wird vollständig in Ihrem Browser erzeugt, ganz ohne Server.",
  },
  {
    label: "Zungenbrecher",
    text: "Fischers Fritze fischt frische Fische; frische Fische fischt Fischers Fritze.",
  },
  { label: "Umlaute", text: "Über den grünen Hügeln blühen im Frühling zahllose schöne Blumen." },
  {
    label: "Ansage",
    text: "Ihr Termin ist für Dienstag um halb zehn bestätigt. Bitte seien Sie pünktlich.",
  },
  {
    label: "Wetter",
    text: "Am Wochenende wird es überwiegend heiter bei Höchstwerten um zweiundzwanzig Grad.",
  },
  {
    label: "Straße & ß",
    text:
      "Die Straße war nass, doch die Füße blieben trocken – zum Glück maßen wir die Größe genau.",
  },
];

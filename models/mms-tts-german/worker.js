// MMS-TTS German (VITS) worker — end-to-end German speech synthesis off the main thread so the control
...
// is no separate vocoder: German text → language-specific token ids → (flows + duration) → 16 kHz
// waveform, all in one network. Nothing here is faked.
//
// This demo is the GERMAN checkpoint (Xenova/mms-tts-deu) — a materially DISTINCT model from the built
// English MMS-TTS demo: its own German character vocabulary and its own VITS weights, trained on German
// speech, so it renders German orthography (umlauts ä/ö/ü, ß, German phonotactics) that the English
// checkpoint cannot. The default modelId is the German one; the worker still accepts any mms-tts-<iso>
// id so a page can compare a second language if it wants.
//
// Because the duration predictor is stochastic, generating the same German text twice yields DIFFERENT
// timing/prosody — that variation is real and is what the "stochastic prosody" surface visualises.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const DEFAULT_MODEL = "Xenova/mms-tts-deu";

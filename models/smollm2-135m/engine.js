// Front-end helpers for the SmolLM2-135M pages: the worker handshake, streaming plumbing, and the
...
//
// SmolLM2-135M-Instruct (HuggingFaceTB/SmolLM2-135M-Instruct) is the smallest SmolLM2 instruct model
// (~135M params, llama-architecture decoder) — a distinct checkpoint from the 360M sibling already in
// this showcase. Transformers.js loads it through the standard text-generation pipeline; it answers
// directly (no <think> block). The worker runs the q8 build on the universal WebAssembly/CPU path
// (~140 MB); WebGPU is an optional accelerator with a WASM fallback.

const WORKER_URL = "/web-ai-showcase/models/smollm2-135m/worker.js";

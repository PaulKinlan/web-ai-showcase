// Front-end helpers for the MMS-TTS (VITS) pages. Thin: owns the worker handshake, WAV encoding, and
// the waveform canvas. All inference runs in worker.js, off the main thread.

const WORKER_URL = "/web-ai-showcase/models/mms-tts/worker.js";

// The subset of MMS languages surfaced in the picker (each is its own ~38 MB VITS checkpoint).
// Meta's MMS-TTS covers 1000+ languages; these are just a browser-friendly sample.
export const LANGUAGES = [
  {
    code: "eng",
    name: "English",
    modelId: "Xenova/mms-tts-eng",
    sample: "Speech synthesised entirely in your browser, no server involved.",
  },
  {
    code: "deu",
    name: "German",
    modelId: "Xenova/mms-tts-deu",
    sample: "Diese Stimme wird vollständig in Ihrem Browser erzeugt.",
  },
  {
    code: "fra",
    name: "French",
    modelId: "Xenova/mms-tts-fra",
    sample: "Cette voix est générée entièrement dans votre navigateur.",
  },
  {
    code: "spa",
    name: "Spanish",
    modelId: "Xenova/mms-tts-spa",
    sample: "Esta voz se genera completamente en tu navegador.",
  },
  {
    code: "por",
    name: "Portuguese",
    modelId: "Xenova/mms-tts-por",
    sample: "Esta voz é gerada inteiramente no seu navegador.",
  },
  {
    code: "rus",
    name: "Russian",
    modelId: "Xenova/mms-tts-rus",
    sample: "Этот голос создаётся полностью в вашем браузере.",
  },
  {
    code: "hin",
    name: "Hindi",
    modelId: "Xenova/mms-tts-hin",
    sample: "यह आवाज़ पूरी तरह आपके ब्राउज़र में बनती है।",
  },
  {
    code: "ara",
    name: "Arabic",
    modelId: "Xenova/mms-tts-ara",
    sample: "هذا الصوت يتم إنشاؤه بالكامل داخل متصفحك.",
  },
  {
    code: "kor",
    name: "Korean",
    modelId: "Xenova/mms-tts-kor",
    sample: "이 목소리는 전적으로 브라우저 안에서 생성됩니다.",
  },
  {
    code: "vie",
    name: "Vietnamese",
    modelId: "Xenova/mms-tts-vie",
    sample: "Giọng nói này được tạo hoàn toàn trong trình duyệt của bạn.",
  },
  {
    code: "yor",
    name: "Yoruba",
    modelId: "Xenova/mms-tts-yor",
    sample: "A ń ṣẹ̀dá ohùn yìí pátápátá nínú ẹ̀rọ aṣàwákiri rẹ.",
  },
];

export class MmsEngine {
  constructor() {
  ...
  load(onProgress, modelId = "Xenova/mms-tts-eng") {
  ...
  /** Synthesize text → { audio: Float32Array, rate, ms, samples, device } */
  speak(text, modelId = "Xenova/mms-tts-eng") {
    const id = ++this._id;
    return new Promise((resolve, reject) => {

// Front-end helpers for the SAM 2.1 pages. Keeps each page thin: it owns the worker handshake
// (load → embed-once → prompt-per-click/box) and the canvas helpers that turn a raw H×W mask into a
// coloured overlay or a transparent cut-out. All inference is in worker.js (off the main thread).

const WORKER_URL = "/web-ai-showcase/models/sam2-segmentation/worker.js";

export class Sam2Engine {
  constructor() {
  ...
  /**
   * Decode candidate masks from prompts.
   * @param {{x:number,y:number,label:number}[]} points normalized [0,1], label 1=fg/0=bg
   * @param {number[]|null} box  [x1,y1,x2,y2] normalized [0,1], or null
   */
  segment(points, box = null) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
      this._pending.set(id, { resolve, reject });
      this.worker.postMessage({ type: "segment", id, points, box });
    });
  }
}
...
export function drawMaskOverlay(canvas, imgEl, overlay, w, h, points = [], box = null, _opts = {}) {
  ...
  if (box && box.length === 4) {
    ...
    ctx.setLineDash([Math.max(6, w / 90), Math.max(4, w / 130)]);
...
.cand-row { display: flex; flex-wrap: wrap; gap: .4rem; margin: .5rem 0; }
.cand-btn {
  font: inherit; font-size: .78rem; padding: .3rem .55rem; border-radius: 8px; cursor: pointer;
  border: 1px solid var(--border); background: var(--bg-raised); color: var(--color);
  display: flex; flex-direction: column; align-items: flex-start; gap: .1rem; min-inline-size: 5.5rem;
}
.cand-btn[aria-pressed=true] { border-color: var(--accent); background: var(--bg-secondary); }
.cand-btn small { color: var(--muted); font-family: var(--font-mono); }
...
.score-bar { display: block; block-size: 6px; border-radius: 3px; background: var(--accent); margin-top: .2rem; }

// Shared front-end helpers for the ViT-GPT2 captioning pages. Owns the worker handshake, turns
// files/samples/webcam frames into data URLs, and streams generated tokens back to the page. All
// inference happens off the main thread in worker.js.

const WORKER_URL = "/web-ai-showcase/models/vit-gpt2-captioning/worker.js";
...
export class CaptionEngine {
  ...
  /**
   * Generate a caption for an image. Streams tokens via onToken({token, t, i}); resolves with
   * { id, caption, tokens, ms, device } when generation finishes.
   */
  caption(imageURL, maxTokens = 30, onToken) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
...
/** Read a File (from upload, drop, or a captured webcam frame) into a data URL. */
...
/** Shared inline styles for the captioning widgets (dropzone, caption box, token trace). */
...
.caption-box {
  font-family: var(--font-display, Georgia, serif); font-size: 1.25rem; line-height: 1.4;
  padding: .8rem 1rem; border-radius: var(--radius); background: var(--bg-raised);
  border: 1px solid var(--border); min-block-size: 2.5em; margin: .5rem 0;
}
.caption-box .cursor { display: inline-block; inline-size: .5ch; background: var(--accent);
  animation: capblink 1s steps(2) infinite; }
...
.code-snippet { font-family: var(--font-mono); font-size: .82rem; background: var(--bg-raised);
  border: 1px solid var(--border); border-radius: var(--radius); padding: .6rem .8rem; overflow-x: auto; }
`;

// Front-end helpers for the wav2vec2-XLSR-53 (Korean) pages. Keeps each page thin: worker handshake,
// audio decode to the 16 kHz mono Float32Array the model wants, a tiny mic recorder, waveform draw, the
// per-frame CTC strip renderer (whole Hangul syllables), and shared CSS. All inference lives in worker.js
// (off the main thread).
//
// DISTINCT from every other ASR demo here: this checkpoint is wav2vec2-large-XLSR-53 fine-tuned for
// KOREAN, and its CTC head emits whole Hangul SYLLABLE blocks from a 1205-symbol vocabulary (가 각 간 …),
// not Latin letters — so the per-frame strip is a stream of complete Korean syllables. The engine is the
// same CTC shape (per-frame argmax → collapse), which lets the page visualise the collapse over Hangul.

const WORKER_URL = "/web-ai-showcase/models/korean-xlsr-asr/worker.js";
const TARGET_RATE = 16000;

export class KoreanXlsrEngine {
  constructor() {
   ...
   *  { text, strip:[{c,blank,boundary}], collapsed:[syllables], words:[{text,start,end}], frameMs, frames, ms, device }. */
 ...
 * emitted Hangul syllables in the accent, a word boundary marked. `strip` = [{ c, blank, boundary }].
 */
...
/** Render per-word forced-alignment chips (Korean word + start–end seconds). */
export function renderWords(el, words) {
  el.replaceChildren(...words.map((w) => {
    const span = document.createElement("span");
    span.className = "align-word";
    span.setAttribute("lang", "ko");
    span.textContent = w.text + " ";
    const t = document.createElement("span");
    t.className = "t";
    t.textContent = `${w.start.toFixed(1)}–${w.end.toFixed(1)}s`;
    span.append(t);
    return span;
  }));
}

/** Korean sample lines (CC0 speech clips bundled with the demo). */
export const KO_SAMPLES = [
  { src: "sample-ko-speech.wav", label: "Korean speech — Lee Jung-jae, award remarks (CC0, 12s)" },
  { src: "sample-ko-gasu.wav", label: "Korean word — “가수” / gasu / “singer” (CC0)" },
];

/** XLSR widget styles, injected once per page (keeps us on the shared design system). Cells are sized a
 *  little wider than the Latin demos because each one holds a whole Hangul syllable block. */
export const KO_XLSR_CSS = `
...
.transcript { font-family:var(--font-body); font-size:1.25rem; line-height:1.9; margin:.4rem 0;
...
.ctc-strip { display:flex; flex-wrap:wrap; gap:2px; padding:.6rem; background:var(--bg-raised);
  ...
  font-size:.9rem; line-height:1.5; max-block-size:12rem; overflow-y:auto; }
.ctc-cell { min-inline-size:1.4em; text-align:center; padding:.05rem .15rem; border-radius:3px;
  color:var(--accent-ink); background:var(--accent); font-weight:600; }
.ctc-cell.blank { color:var(--muted); background:transparent; font-weight:400; min-inline-size:1em; }
.ctc-cell.bound { color:var(--color); background:color-mix(in srgb, var(--accent) 18%, transparent); }
.ctc-collapsed { font-family:var(--font-body); font-size:1.3rem; letter-spacing:.02em; margin:.4rem 0;
...
.align-word { display:inline-block; padding:.2rem .55rem; border-radius:999px;
  border:1px solid var(--border); background:var(--bg-raised); font-family:var(--font-body); font-size:1rem; }

// Front-end helpers for the YOLOv10 pages. Thin: owns the worker handshake (transferring ImageBitmaps
// so webcam frames are zero-copy), draws the box overlay on a canvas, and renders the box table.
// All inference + letterbox + head-decode lives in worker.js (off the main thread, raw ONNX Runtime Web).

const WORKER_URL = "/web-ai-showcase/models/yolov10-detection/worker.js";
...
export class YoloEngine {
  ...
  /** Detect on an ImageBitmap (transferred → zero-copy). Returns {detections, ms, device, imgW, imgH}. */
  detect(bitmap) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
...
/** Create an ImageBitmap from a File/Blob/HTMLImageElement/HTMLVideoElement/Canvas. */
 ...
 * CSS scales the canvas down responsively, so boxes stay pixel-accurate. Boxes are in the source's
 * pixel coordinates. `highlightIndex` thickens one box (used by the multi-model crop picker).
 */

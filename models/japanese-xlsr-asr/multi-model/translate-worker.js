// Japanese→English translation worker for the Japanese XLSR multi-model demo. Runs OPUS-MT (MarianMT)
// off the main thread so the ASR→translate chain never blocks the UI.
// Model: Xenova/opus-mt-ja-en (task: translation). Same MarianMT class as the built marianmt-translation
// demo, Japanese→English language pair.
...
async function ensureLoaded() {
  ...
  translator = await pipeline("translation", MODEL_ID, {
    device: DEVICE,
    dtype: "q8", // onnx/*_quantized — verified to translate ja→en on WASM
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[opus-mt-ja worker]", err);

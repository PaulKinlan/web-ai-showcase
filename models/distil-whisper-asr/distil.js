// Front-end helpers for the Distil-Whisper pages. Keeps each page thin: worker handshake, audio decode
// to the 16 kHz mono Float32Array the model wants, a tiny mic recorder, waveform draw, and shared CSS.
// All inference lives in worker.js (off the main thread).
//
// Distil-Whisper is Whisper *knowledge-distilled*: it keeps the full Whisper encoder but shrinks the
// decoder (distil-small.en: 12 encoder layers, only 4 decoder layers vs whisper-small's 12). Same
// seq2seq architecture and the same timestamp support as Whisper — just a lot fewer decoder passes, so
// it transcribes faster. That speed is the story, so we measure and report a real real-time factor.

const WORKER_URL = "/web-ai-showcase/models/distil-whisper-asr/worker.js";
const TARGET_RATE = 16000;

export class DistilEngine {
  constructor() {
   ...
   *  Returns { text, segments, tokens, tokPerSec, ms, rtf, speedup, audioSec, device }. */
...
/** Distil-Whisper widget styles, injected once per page (keeps us on the shared design system). */
export const DISTIL_CSS = `
...
.caption-box { min-block-size:3rem; font-family:var(--font-body); font-size:1.15rem; line-height:1.6;
  padding:.8rem; background:var(--bg-raised); border:1px solid var(--border); border-radius:var(--radius); }
.batch-row { display:flex; flex-wrap:wrap; gap:.5rem; align-items:center; margin:.4rem 0;
  padding:.5rem .6rem; border:1px solid var(--border); border-radius:var(--radius); background:var(--bg-raised); }
.batch-row .b-label { font-family:var(--font-mono); font-size:.78rem; color:var(--muted); min-inline-size:7rem; }
.batch-row .b-text { flex:1 1 12rem; }
.batch-row .b-badge { font-family:var(--font-mono); font-size:.72rem; color:var(--accent-ink);
  background:var(--accent); padding:.1rem .45rem; border-radius:999px; }
`;

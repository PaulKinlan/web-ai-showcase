// Front-end helpers shared by every Face-Parsing page. Owns the worker handshake, turns files/samples
// into data URLs, and paints the colour-coded face-part map (a per-pixel class overlay) plus the
// legend and per-class coverage bars. All inference lives in worker.js (off the main thread).
//
// Model: Xenova/face-parsing (task: image-segmentation; SegFormer finetuned on CelebAMask-HQ). This is
// DENSE SEMANTIC segmentation over 19 FACE-PART classes — every pixel becomes skin / hair / eye /
// brow / lip / nose / ear / neck / clothing … — a different job from portrait matting (one subject
// cut-out, RMBG/MODNet), scene semantics (SegFormer-ADE), or point-prompted masks (SAM).

const WORKER_URL = "/web-ai-showcase/models/face-parsing/worker.js";

export class FaceParsingEngine {
  constructor() {
 ...
 * (OffscreenCanvas.transferToImageBitmap) — the fast path here is a bare drawImage(bitmap). When only
 * the raw RGBA buffer is available (worker without OffscreenCanvas, or a freshly-filtered buffer), we
 * fall back to a one-off putImageData at MAP resolution (invariant 15 measured fallback).
 */
...
/**
 * Paint the face-part map over the photo. `overlay` is EITHER the worker-composited ImageBitmap
 ...
 */
export function paintFaceMap(canvas, img, overlay, mapW, mapH, opacity = 0.6) {
  const w = img.naturalWidth, h = img.naturalHeight;
  canvas.width = w;
  canvas.height = h;
  ...
  ctx.imageSmoothingEnabled = false; // crisp part boundaries, not blurred
  ctx.drawImage(src, 0, 0, w, h);
  ctx.globalAlpha = 1;
}

/** Paint just the colour-blocked face map (no photo) — "turn a face into a part-map". */
...
/**
 * Upscale the class-colour overlay to a target size with nearest-neighbour (crisp part edges) and
...
/**
 * Tint one or more face-part classes with a colour, keeping the photo everywhere else — the "digital
 * makeup / recolour" compositor. `targets` is an array of {rgb:[r,g,b], color:"#hex", alpha}. Draws
 * into `canvas` at photo resolution. Pixels whose winning class colour matches a target get that
 * target's colour blended over the original photo pixel (multiply-ish tint via globalAlpha).
 */
export function tintClasses(canvas, photo, overlay, mapW, mapH, targets) {
  ...
  const d = base.data;
  // Build a fast lookup from "r,g,b" overlay colour → {tint rgb, alpha}
...
.swatch-btn { display:inline-flex; align-items:center; gap:.4rem; padding:.4rem .7rem; min-block-size:40px; border:1px solid var(--border); border-radius:999px; background:var(--bg-raised); color:var(--color); cursor:pointer; font-size:.82rem; }

...
const WORKER_URL = "/web-ai-showcase/models/trocr-handwriting/worker.js";
...
/**
 * A handwriting drawing pad: write a line with mouse / touch / pen, then hand the strokes to TrOCR
 * as a PNG data URL. White background + dark ink matches the IAM handwriting domain. Pointer Events
 * cover mouse, touch, and pen with one code path; `touch-action: none` keeps touch drawing from
 * scrolling the page. Keyboard alternative: an <input>-driven "type it in script" path is offered
 * alongside by each page (the pad itself is a bitmap, so pages label it and mirror the recognised
 * text into a live region).
 */
export class DrawPad {
  constructor(canvas) {
    ...
    canvas.addEventListener("pointerdown", (ev) => {
      ...
      this._drawing = true;
      this._last = pos(ev);
      this._dot(this._last);
    });
    canvas.addEventListener("pointermove", (ev) => {
      if (!this._drawing) return;
      const p = pos(ev);
      this._line(this._last, p);
      this._last = p;
    });
    const end = () => {
      if (!this._drawing) return;
  ...
  _resize() {
    const w = Math.min(560, Math.max(280, this.canvas.clientWidth || 480));
    this.canvas.width = w;
    this.canvas.height = Math.round(w * 0.36);
    this.clear();
  }
  _ink() {
    const ctx = this.ctx;
    ...
    ctx.lineWidth = Math.max(3, this.canvas.width / 160);
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
  }
  _dot(p) {
  ...
  /** The written line as a PNG data URL — exactly what the model sees. */
  toDataURL() {
...
.draw-wrap { position: relative; max-inline-size: 100%; }
.draw-wrap canvas {
  inline-size: 100%; max-inline-size: 560px; block-size: auto; border-radius: 8px; display: block;
  background: #fff; border: 1px solid var(--border); cursor: crosshair; touch-action: none;
}
.draw-wrap canvas:focus-visible { outline: 2px solid var(--accent); outline-offset: 2px; }
`;

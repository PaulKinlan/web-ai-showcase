// Front-end helpers for the Whisper pages. Keeps each page thin: it owns the worker handshake, turns
// samples / uploads / mic recordings into the 16 kHz mono Float32Array Whisper wants, draws the
// waveform, and renders the timestamped transcript. All inference lives in worker.js (off-thread).

const WORKER_URL = "/web-ai-showcase/models/whisper-speech-to-text/worker.js";
...
/** Fetch an audio URL and decode it to 16 kHz mono. */
export async function urlToMono16k(url) {
  const buf = await (await fetch(url)).arrayBuffer();
  return decodeToMono16k(buf);
}

/** Decode a Blob (mic recording or upload) to 16 kHz mono. */
...
/** Format seconds as m:ss.d for the timestamp column. */
...
/**
 * Draw a mono waveform into a <canvas>. `progress` (0..1) highlights the played portion in the accent
 * colour. Reads the CSS custom properties so it matches light/dark automatically.
 */
...
/** Whisper-specific widget styles, injected once per page (keeps us on the shared design system). */
export const WHISPER_CSS = `
...
.seg-row.active td { background:color-mix(in srgb, var(--accent) 14%, transparent); }
.karaoke { font-family:var(--font-display); font-size:1.5rem; line-height:1.7; }
.karaoke .w { transition:color .12s, background .12s; padding:0 .05em; border-radius:4px; color:var(--muted); }
.karaoke .w.on { color:var(--accent-ink); background:var(--accent); }
.karaoke .w.done { color:var(--color); }

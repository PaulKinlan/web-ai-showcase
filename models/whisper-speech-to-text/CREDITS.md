# Audio credits — Whisper speech-to-text

## LibriSpeech validation sample

- **File:** `libri-61-70968-0000-16k-mono.wav`
- **Corpus:** LibriSpeech `test-clean`, utterance `61-70968-0000`
- **Reader / work:** Paul-Gabriel Wiener reading *Robin Hood*, chapter 4
- **Corpus creators:** Vassil Panayotov, Guoguo Chen, Daniel Povey, Sanjeev Khudanpur
- **Source:** [OpenSLR SLR12](https://www.openslr.org/12), `test-clean.tar.gz`
- **Licence:** [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
- **Derivation:** Lossless PCM conversion from `test-clean/61/70968/61-70968-0000.flac` to 16 kHz mono 16-bit WAV; no content edit
- **SHA-256:** `1fb553adb5a6389eef5b7ebdbf9ed2a6082518a646ce9bf872bf33b964eedc14`
- **Reference transcript:** “He began a confused complaint against the wizard, who had vanished behind the curtain on the left.”

## JFK historical speech sample

- **File:** `jfk.wav`
- **Content:** John F. Kennedy’s inaugural address, January 20, 1961 — excerpt beginning “And so, my fellow Americans…”
- **Source mirror:** [Xenova/transformers.js-docs](https://huggingface.co/datasets/Xenova/transformers.js-docs/resolve/main/jfk.wav)
- **Use here:** Faithfully labelled, neutral historical automatic-speech-recognition and timestamp testing only
- **SHA-256:** `627f0e49f927ffcd4120ed60a035ff2f8d448e2e7469452c7ecaffed06fe135b`

The rights-unclear TED preset is no longer offered by the Whisper pages. Upload remains available for visitors’ own audio. The legacy asset path is temporarily retained only because two published cross-family routes still depend on it; their migration is tracked separately.

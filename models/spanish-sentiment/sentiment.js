// Front-end helpers for the spanish-sentiment pages. Keeps each page thin: it owns the worker
// handshake and the renderers (3-class probability bars, word-level occlusion attribution, a compact
// verdict pill). ALL inference lives in worker.js (off the main thread).
//
// Model: Xenova/robertuito-sentiment-analysis — an ONNX export of pysentimiento/robertuito-sentiment-
// analysis. RoBERTuito is a RoBERTa pre-trained ONLY on ~500M Spanish tweets (user/hashtag/emoji aware),
// then fine-tuned for sentiment on the TASS corpus. This is a SPANISH-NATIVE classifier — its whole
// vocabulary and pre-training are Spanish, distinct from the English DistilBERT SST-2 demo and from the
// ~8-language XLM-R multilingual demo (which spreads capacity across 100 languages). Classes come from
// the model config id2label: 0 NEG · 1 NEU · 2 POS.

const WORKER_URL = "/web-ai-showcase/models/spanish-sentiment/worker.js";

// The model emits NEG / NEU / POS (RoBERTuito / TASS label set). Map to our internal keys once, here.
export const LABEL_MAP = { NEG: "negative", NEU: "neutral", POS: "positive" };

export const CLASS_META = {
  negative: { label: "Negativo", en: "Negative", emoji: "🙁", varName: "--bad" },
  neutral: { label: "Neutral", en: "Neutral", emoji: "😐", varName: "--warn" },
  positive: { label: "Positivo", en: "Positive", emoji: "🙂", varName: "--good" },
};
...
/** A compact verdict pill (emoji + Spanish label + confidence). */

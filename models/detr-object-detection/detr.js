// Shared front-end helpers for the DETR pages. Keeps each page thin: it owns the worker handshake,
// turns files/samples into data URLs, draws the box overlay on a canvas, and renders the box table.
// All inference is in worker.js (off the main thread).

const WORKER_URL = "/web-ai-showcase/models/detr-object-detection/worker.js";

export class DetrEngine {
  constructor() {
 ...
 * source image's pixel coordinates. Returns the drawn boxes with their on-canvas rects for
 * hit-testing (used by the multi-model page).
 */
...
/** Shared inline styles for the DETR-specific widgets (canvas stage, dropzone, box table). */
export const DETR_CSS = `
.dropzone {

// wav2vec2 (Dutch) CTC worker — ALL inference off the main thread.
//
// DISTINCT model + language from every other ASR demo in this showcase. This is a DUTCH speech
// recogniser: facebook/wav2vec2-base-10k-voxpopuli fine-tuned on Dutch, exported to ONNX by the
...
// Slovak VoxPopuli) are CTC too, but none is Dutch.
//
// HONEST NOTE ON THE RUNTIME. Like the Czech/Slovak/Croatian/Romanian/Hungarian VoxPopuli demos (and
// unlike the Polish/Finnish onnx-community mirrors that load straight through transformers.js as
// AutoModelForCTC), the ONLY browser ONNX export of the Dutch VoxPopuli checkpoint is
// OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-nl-onnx, packaged for the onnx-asr toolkit: its
...
// in-browser run of the genuine Dutch Meta-AI checkpoint — not a relabel of another language, not a fake.
//
// NOT the wav2vec2-large-XLSR-53 Dutch model: jonatasgrosman/wav2vec2-large-xlsr-53-dutch is
// PyTorch/Flax-only (no browser ONNX — recorded blocked in this catalogue), so this demo honestly uses
// the smaller wav2vec2-base VoxPopuli Dutch model that DOES ship a real, runnable CTC ONNX.
//
...
// headless Chrome (ORT Web 1.20.1, WASM EP) to produce real log-probs [1, frames, 41] and a coherent
// Dutch transcript on a CC0 VoxPopuli sample. Weights are fetched from the canonical HF repo and cached in
...
const MODEL_ID = "OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-nl-onnx";
const MODEL_BASE = `https://huggingface.co/${MODEL_ID}/resolve/main/`;
const VOCAB_URL = "/web-ai-showcase/models/dutch-voxpopuli-asr/vocab.txt";
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[wav2vec2-nl worker]", err);

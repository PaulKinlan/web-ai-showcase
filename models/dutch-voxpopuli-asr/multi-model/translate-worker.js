// Dutch→English translation worker for the Dutch wav2vec2 multi-model demo. Runs OPUS-MT (MarianMT) off
// the main thread so the ASR→translate chain never blocks the UI.
// Model: Xenova/opus-mt-nl-en (task: translation) — the dedicated Helsinki-NLP Dutch→English MarianMT,
// which ships a full transformers.js ONNX export. Same MarianMT class as the built marianmt-translation demo.
...
async function ensureLoaded() {
  ...
  translator = await pipeline("translation", MODEL_ID, {
    device: DEVICE,
    dtype: "q8", // onnx/*_quantized — verified to translate nl→en correctly on WASM
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[opus-mt-nl worker]", err);

// Front-end helpers for the Dutch wav2vec2 ASR pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT language from every other ASR demo in this showcase: this is DUTCH — wav2vec2-base-10k-
// voxpopuli fine-tuned on Dutch (OpenVoiceOS ONNX export), a Latin-script CTC recogniser over a
// 41-symbol vocab with the Dutch diacritics (à ç è é ê ë í ï ó ö ü). NOT the wav2vec2-large-
// XLSR-53 model (no Dutch XLSR-53 ONNX exists) — see the model page's at-a-glance. Because the only
// Dutch ONNX export is an onnx-asr graph transformers.js can't load (model_type "wav2vec2-ctc"), the
// worker runs it directly on ONNX Runtime Web. The engine is the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the Dutch alphabet.

const WORKER_URL = "/web-ai-showcase/models/dutch-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class DutchAsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Dutch word + start–end seconds). */

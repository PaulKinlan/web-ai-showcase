// wav2vec2 (Croatian) CTC worker — ALL inference off the main thread.
//
// DISTINCT model + language from every other ASR demo in this showcase. This is a CROATIAN speech
// recogniser: facebook/wav2vec2-base-10k-voxpopuli fine-tuned on Croatian, exported to ONNX by the
// OpenVoiceOS project. All the built ASR demos (English wav2vec2, Russian/French/Spanish/Italian/
// Portuguese/Chinese/Japanese/Korean/Thai XLSR + Finnish/Polish VoxPopuli) are CTC too, but none is Croatian.
//
// HONEST NOTE ON THE RUNTIME. Unlike the Polish/Finnish VoxPopuli demos (onnx-community mirrors that
// load straight through transformers.js as AutoModelForCTC), the ONLY browser ONNX export of the Croatian
// VoxPopuli checkpoint is OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-hr-onnx, which is packaged for the
// onnx-asr toolkit: its config.json declares `model_type: "wav2vec2-ctc"` (a type transformers.js does
// NOT register → "Unsupported model type: wav2vec2-ctc"), the graph takes `input_values` + `input_lengths`
// and emits `logprobs` (log-softmax) rather than `logits`, and feature normalisation is baked INTO the
// graph. So this demo runs the raw ONNX graph directly on ONNX Runtime Web (the same engine transformers.js
// uses under the hood), feeding raw 16 kHz audio and CTC-decoding the log-probs ourselves. It is a real,
// in-browser run of the genuine Croatian Meta-AI checkpoint — not a relabel of another language and not a fake.
//
...
// headless Chrome (ORT Web 1.20.1, WASM EP) to produce real log-probs [1, frames, 39] and a coherent Croatian
// transcript on a Public-Domain sample. Weights are fetched from the canonical HF repo and cached in Cache
// Storage under the repo URL so the shared model-cache layer detects them (auto-init on return visits).
...
const MODEL_ID = "OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-hr-onnx";
const MODEL_BASE = `https://huggingface.co/${MODEL_ID}/resolve/main/`;
const VOCAB_URL = "/web-ai-showcase/models/croatian-voxpopuli-asr/vocab.txt";
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[wav2vec2-hr worker]", err);

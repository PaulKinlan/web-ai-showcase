// Front-end helpers for the Croatian wav2vec2 ASR pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT language from every other ASR demo in this showcase: this is CROATIAN — wav2vec2-base-10k-
// voxpopuli fine-tuned on Croatian (OpenVoiceOS ONNX export), a Latin-script CTC recogniser over a
// 39-symbol vocab with the Croatian diacritics (č ć đ š ž). NOT the wav2vec2-large-
// XLSR-53 model (no Croatian XLSR-53 ONNX exists) — see the model page's at-a-glance. Because the only
// Croatian ONNX export is an onnx-asr graph transformers.js can't load (model_type "wav2vec2-ctc"), the
// worker runs it directly on ONNX Runtime Web. The engine is the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the Croatian alphabet.

const WORKER_URL = "/web-ai-showcase/models/croatian-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class CroatianAsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Croatian word + start–end seconds). */

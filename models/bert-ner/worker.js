// BERT NER worker — inference off the main thread so the control UI stays responsive.
// Model: Xenova/bert-base-NER (task: token-classification), WASM, q8. ONNX build of dslim/bert-base-NER.
//
// The pipeline tags each WordPiece token with a BIO label — B-PER/I-PER, B-ORG/I-ORG, B-LOC/I-LOC,
// B-MISC/I-MISC — and a confidence, but it does NOT return character offsets. So here we:
//   1. run token-classification to get per-token {entity, score, index, word},
//   2. map each token back to a character span in the ORIGINAL text (cursor-walk indexOf, so
//      highlighting lands on the real substring even with casing/punctuation),

// Front-end helpers shared by every Gemma-3-1B (WebLLM) page: the worker handshake + streaming
// plumbing, the honest WebGPU gate/fallback, Gemma's system-prompt folding, and the chat widget CSS.
...
//
// Gemma 3 keeps Gemma's turn-based template (<start_of_turn>user / <start_of_turn>model) with NO
// dedicated system role, so a "system prompt" is folded into the first user turn — identical handling
// to Gemma 2. Gemma 3 1B is a newer, distinct architecture (a longer-context, restructured successor);
// this page is the smallest capable WebLLM chat model in the catalogue, reaching more devices.

export { webGPUAdapterAvailable } from "/web-ai-showcase/lib/webllm.js";

const WORKER_URL = "/web-ai-showcase/models/gemma-3-webllm/worker.js";

export class GemmaChatEngine {
  constructor() {
...
/**
 * Gemma 3's chat template has NO dedicated system role — only alternating user/model turns
 * (<start_of_turn>user … <start_of_turn>model). So a "system prompt" is folded into the first user
 * turn. This returns a NEW messages array with any system content prepended to the first user
 * message, guaranteeing a valid Gemma conversation across WebLLM versions. Display THIS (not the
 * pre-fold array) as the wire, so the "see inside" surface is honest about what the model receives.
 */
export function foldSystemForGemma(messages) {
  ...
  return rest.map((m, i) =>
    i === firstUserIdx ? { role: m.role, content: `${systemText}\n\n${m.content}` } : m
  );
}
 ...
 *  the ~0.9 GB download so a visitor knows what running it for real would cost. */
export function webgpuFallbackHTML(probe) {
  ...
  return `
    <strong>Gemma 3 1B runs on WebLLM, which needs WebGPU — and it isn't available in this browser.</strong>
    <p class="muted" style="margin:.4rem 0">${why}${
    probe.detail ? " (" + escapeHTML(probe.detail) + ")" : ""
  }
    WebLLM has <em>no WASM fallback</em>: even a 1B model decoding token-by-token needs a real GPU, and the
    weights are a <strong>~0.9 GB</strong> download (cached after the first load). So this page won't fake a
    reply. To run it for real:</p>
    <ul class="muted" style="margin:.2rem 0">
      ...
      <li>The q4f16_1 build is fastest on GPUs that report the <code>shader-f16</code> feature (most modern ones), and needs under ~1 GB of GPU memory — the lightest chat model here.</li>
    </ul>`;
}

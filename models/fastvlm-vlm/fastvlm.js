// Front-end helpers for the FastVLM pages: the worker handshake, streaming plumbing, a real WebGPU
// probe (for the honest fallback readout), image helpers, and the widget CSS. Inference is in
// worker.js. Model: onnx-community/FastVLM-0.5B-ONNX (image-text-to-text) — Apple's FastViTHD vision
// encoder + Qwen2-0.5B decoder, small enough to run on WebGPU (q4f16) OR the WASM CPU fallback (q4).

const WORKER_URL = "/web-ai-showcase/models/fastvlm-vlm/worker.js";

export class FastVLMEngine {
  constructor() {
...
.chip { font:inherit; font-size:.78rem; padding:.3rem .6rem; border-radius:999px; min-block-size:34px;

// Front-end helpers for the Phi-3.5-vision pages: the worker handshake, streaming plumbing, a real
// WebGPU probe (for the honest fallback), image helpers, and the widget CSS. Inference is in worker.js.

const WORKER_URL = "/web-ai-showcase/models/phi-3.5-vision/worker.js";

export class Phi3VisionEngine {
  constructor() {
  ...
  /**
   * Stream a generation. `images` is one data URL or an array of them (multi-image compare).
   * onToken(token, tMs) fires per token; onPrompt(template) once; onMeta({imageTokens,promptLen,images}) once.
   */
  generate(images, prompt, { maxTokens = 256, onToken, onPrompt, onMeta } = {}) {
    const id = ++this._id;
    const imgs = Array.isArray(images) ? images : [images];
    return new Promise((resolve, reject) => {
      this._active = { id, onToken, onPrompt, onMeta, resolve, reject };
      this.worker.postMessage({ type: "run", id, images: imgs, prompt, maxTokens });
    });
  }
}
...
.chip { font:inherit; font-size:.78rem; padding:.35rem .7rem; border-radius:999px; min-block-size:36px;
...
.multi-strip { display:flex; gap:.5rem; flex-wrap:wrap; }
.multi-slot { position:relative; inline-size:130px; }
.multi-slot img { inline-size:130px; block-size:100px; object-fit:cover; border-radius:8px; border:1px solid var(--border); display:block; }
.multi-slot .badge { position:absolute; inset-block-start:4px; inset-inline-start:4px; background:var(--accent);
  color:#fff; font-size:.66rem; padding:.05rem .35rem; border-radius:4px; }
`;

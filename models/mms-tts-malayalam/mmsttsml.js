// Front-end helpers for the MMS-TTS Malayalam (VITS) pages. Thin: owns the worker handshake, WAV
// encoding and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: naklitechie/mms-tts-ml-ONNX — a faithful ONNX export of the canonical Meta MMS MALAYALAM VITS
// checkpoint (its vocab.json is byte-identical to facebook/mms-tts-mal). ~114 MB fp32, 16 kHz mono
// output. This is a DISTINCT model from the built English/German/Spanish/French/Arabic/Vietnamese/Hindi/
// Tamil/Gujarati/Telugu/Marathi/Punjabi MMS-TTS demos: its own native Malayalam-script char vocab (84
// symbols, is_uroman:false — a different script from Devanagari and Tamil) + Malayalam-trained VITS
// weights. The worker reproduces the VitsTokenizer exactly (verified identical to the real tokenizer)
// because this export omits tokenizer.json.

const WORKER_URL = "/web-ai-showcase/models/mms-tts-malayalam/worker.js";
export const MODEL_ID = "naklitechie/mms-tts-ml-ONNX";

export class MmsMlEngine {
  constructor() {
  ...
  /** Synthesize Malayalam text → { audio: Float32Array, rate, ms, samples, device, inputIds } */
...
// A small set of Malayalam (Malayalam script) example lines that exercise vowel signs, chillu and
// conjuncts.
export const MALAYALAM_SAMPLES = [
  {
    label: "ആമുഖം",
    text: "ഈ ശബ്ദം പൂർണമായും നിങ്ങളുടെ ബ്രൗസറിൽ, ഒരു സെർവറും ഇല്ലാതെ സൃഷ്ടിക്കപ്പെടുന്നു.",
  },
  {
    label: "നമസ്കാരം",
    text: "നമസ്കാരം! സുഖമാണോ? നിങ്ങളെ കണ്ടതിൽ വളരെ സന്തോഷം.",
  },
  {
    label: "കാലാവസ്ഥ",
    text: "ഈ വാരാന്ത്യത്തിൽ കാലാവസ്ഥ കൂടുതലും തെളിഞ്ഞതായിരിക്കും, പരമാവധി താപനില മുപ്പത്തിരണ്ട് ഡിഗ്രി.",
  },
  {
    label: "വാർത്ത",
    text: "ഭാഷാ മാതൃക പൂർണമായും ബ്രൗസറിൽ പ്രവർത്തിപ്പിക്കുന്ന പുതിയ രീതി ഗവേഷകർ അവതരിപ്പിച്ചു.",
  },
  {
    label: "അറിയിപ്പ്",
    text: "ദയവായി ശ്രദ്ധിക്കുക: കൊച്ചിയിലേക്കുള്ള ട്രെയിൻ ഇന്ന് അഞ്ചാം പ്ലാറ്റ്ഫോമിൽ നിന്ന് പുറപ്പെടും.",
  },
  {
    label: "പഴഞ്ചൊല്ല്",
    text: "അറിവ് ഒരു ദിവസം കൊണ്ട് വളരുന്നതല്ല; നിരന്തരമായ പഠനമാണ് അതിന്റെ വഴി.",
  },
];

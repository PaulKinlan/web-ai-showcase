// BGE-reranker-v2-m3 cross-encoder worker — inference off the main thread.
// Model: onnx-community/bge-reranker-v2-m3-ONNX (XLMRobertaForSequenceClassification, num_labels=1),
// WASM, q8 (→ model_quantized.onnx, ~571 MB).
//
// This is BAAI's SECOND-generation multilingual reranker, built on the M3 backbone (XLM-RoBERTa-LARGE,
// 100+ languages) — a step up from the v1 bge-reranker-base (XLM-RoBERTa-base). Like every cross-encoder
// it reads the query and a passage TOGETHER ([CLS] query [SEP] passage [SEP]) and emits ONE relevance
// logit — attention flows across both texts, so it judges relevance far better than comparing two
// independent embeddings, and it does so ACROSS languages (an English query can rank a Chinese passage).
// We read the RAW logit straight off the model, rank passages by it, and also compute a naive

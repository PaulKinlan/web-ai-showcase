// wav2vec2 (Hungarian) CTC worker — ALL inference off the main thread.
//
// DISTINCT model + language from every other ASR demo in this showcase. This is a HUNGARIAN speech
// recogniser: facebook/wav2vec2-base-10k-voxpopuli fine-tuned on Hungarian, exported to ONNX by the
// OpenVoiceOS project. All the built ASR demos (English wav2vec2, Russian/French/Spanish/Italian/
// Portuguese/Chinese/Japanese/Korean/Thai XLSR + Finnish/Polish/Czech/Croatian/Romanian VoxPopuli) are
// CTC too, but none is Hungarian.
//
// HONEST NOTE ON THE RUNTIME. Like the Czech/Croatian/Romanian VoxPopuli demos (and unlike the
// Polish/Finnish onnx-community mirrors that load straight through transformers.js as AutoModelForCTC),
// the ONLY browser ONNX export of the Hungarian VoxPopuli checkpoint is
// OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-hu-onnx, which is packaged for the onnx-asr toolkit: its
...
// `logprobs` (log-softmax) rather than `logits`, and feature normalisation is baked INTO the graph. So this
// demo runs the raw ONNX graph directly on ONNX Runtime Web (the same engine transformers.js uses under the
// hood), feeding raw 16 kHz audio and CTC-decoding the log-probs ourselves. It is a real, in-browser run of
// the genuine Hungarian Meta-AI checkpoint — not a relabel of another language and not a fake.
//
...
// headless Chrome (ORT Web 1.20.1, WASM EP) to produce real log-probs [1, frames, 39] and a coherent
// Hungarian transcript on a CC0 VoxPopuli sample. Weights are fetched from the canonical HF repo and cached
...
const MODEL_ID = "OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-hu-onnx";
const MODEL_BASE = `https://huggingface.co/${MODEL_ID}/resolve/main/`;
const VOCAB_URL = "/web-ai-showcase/models/hungarian-voxpopuli-asr/vocab.txt";
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[wav2vec2-hu worker]", err);

// Front-end helpers for the Hungarian wav2vec2 ASR pages. Keeps each page thin: worker handshake,
// audio decode to the 16 kHz mono Float32Array the model wants, a tiny mic recorder, waveform draw, the
// per-frame CTC strip renderer, and shared CSS. All inference lives in worker.js (off the main thread).
//
// DISTINCT language from every other ASR demo in this showcase: this is HUNGARIAN — wav2vec2-base-10k-
// voxpopuli fine-tuned on Hungarian (OpenVoiceOS ONNX export), a Latin-script CTC recogniser over a
// 39-symbol vocab with the Hungarian diacritics (á é í ó ö ő ú ü ű). NOT the wav2vec2-large-
// XLSR-53 model (no Hungarian XLSR-53 ONNX exists) — see the model page's at-a-glance. Because the only
// Hungarian ONNX export is an onnx-asr graph transformers.js can't load (model_type "wav2vec2-ctc"), the
// worker runs it directly on ONNX Runtime Web. The engine is the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the Hungarian alphabet.

const WORKER_URL = "/web-ai-showcase/models/hungarian-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class HungarianAsrEngine {
  constructor() {
   ...
   *  { text, strip:[{c,blank,boundary}], collapsed:[chars], words:[{text,start,end}], frameMs, frames, ms, device }. */
...
/** Render per-word forced-alignment chips (Hungarian word + start–end seconds). */
...
/** XLSR widget styles, injected once per page (keeps us on the shared design system). */
export const HU_ASR_CSS = `
...
.transcript { font-family:var(--font-body); font-size:1.15rem; line-height:1.8; margin:.4rem 0;
...
.ctc-strip { display:flex; flex-wrap:wrap; gap:2px; padding:.6rem; background:var(--bg-raised);
  ...
  font-size:.82rem; line-height:1.4; max-block-size:11rem; overflow-y:auto; }
.ctc-cell { min-inline-size:1.05em; text-align:center; padding:.05rem .1rem; border-radius:3px;
...
.ctc-collapsed { font-family:var(--font-mono); font-size:1.2rem; letter-spacing:.03em; margin:.4rem 0;
...
.align-word { display:inline-block; padding:.2rem .55rem; border-radius:999px;
  border:1px solid var(--border); background:var(--bg-raised); font-family:var(--font-mono); font-size:.9rem; }
.align-word .t { color:var(--muted); font-size:.7rem; margin-inline-start:.35rem; }
`;

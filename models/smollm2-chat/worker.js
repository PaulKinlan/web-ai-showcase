// SmolLM2-360M-Instruct worker — text generation off the main thread, auto-picking the backend.
// Model: HuggingFaceTB/SmolLM2-360M-Instruct (task: text-generation), which ships an ONNX build.
// SmolLM2-360M is small enough to run for real on the CPU: we prefer WebGPU + dtype q4f16 (4-bit
// weights, fp16 compute) when a real GPU adapter exists, and fall back to WebAssembly + q8 otherwise.
// Either path is real inference — nothing is faked. Uses the canonical Transformers.js text-generation
...
async function ensureLoaded(device, dtype) {
  ...
  loadedDevice = dev;
  console.log(`[smollm2 worker] ready on ${dev}`);
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[smollm2 worker] error", err);

// Front-end helpers shared by every Qwen3-1.7B (WebLLM) page: the worker handshake + streaming
// plumbing, the honest WebGPU gate/fallback, Qwen3's hybrid-thinking switch, and the chat widget CSS.
// All real inference happens in worker.js, which drives lib/webllm.js (MLC's WebGPU engine). We
// re-export the canonical webGPUAdapterAvailable() so pages gate on the exact helper the task mandates.
//
// What makes Qwen3 distinct: it is a HYBRID reasoning model. In "thinking" mode it emits an explicit
// <think>…</think> chain-of-thought before the answer; in "non-thinking" mode it answers directly.
// The mode is toggled with the /think and /no_think soft switches appended to the user turn (a Qwen3
// chat-template convention). The worker separates the reasoning stream from the answer stream so the
// UI can show the model's private reasoning trace as a distinct, collapsible surface.

export { webGPUAdapterAvailable } from "/web-ai-showcase/lib/webllm.js";

const WORKER_URL = "/web-ai-showcase/models/qwen3-webllm/worker.js";

export class Qwen3ChatEngine {
  constructor() {
    this.worker = new Worker(WORKER_URL, { type: "module" });
    this.ready = false;
    this.onProgress = null;
    this._loadWaiters = [];
    this._active = null; // { id, onToken, onReasoning, onFirstToken, resolve, reject }
    this._id = 0;
  ...
  /**
   * Stream a chat completion. `req` = { messages, temperature, top_p, max_tokens, thinking }.
   * onToken(delta) fires per answer chunk; onReasoning(delta) per reasoning chunk; onFirstToken(ttftMs)
   * once, at the first token of either stream.
   */
  chat(req, { onToken, onReasoning, onFirstToken } = {}) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
      this._active = { id, onToken, onReasoning, onFirstToken, resolve, reject };
...
/**
 * Qwen3's hybrid mode is selected with the /think and /no_think soft switches, appended to the LAST
 * user turn (a documented Qwen3 chat-template convention that WebLLM honours). Qwen3 DOES have a real
 * system role, so — unlike Gemma/Mistral — no system-prompt folding is needed. Returns a NEW messages
 * array with the switch appended to the final user message. Display THIS as the wire so the "see
 * inside" surface is honest about exactly what the model receives.
 */
export function applyThinkingSwitch(messages, thinking) {
  const sw = thinking ? "/think" : "/no_think";
  const out = messages.map((m) => ({ role: m.role, content: m.content }));
  for (let i = out.length - 1; i >= 0; i--) {
    if (out[i].role === "user") {
      out[i].content = `${out[i].content} ${sw}`;
      return out;
    }
  }
  out.push({ role: "user", content: sw });
  return out;
}
 ...
 *  the ~1.1 GB download so a visitor knows what running it for real would cost. */
export function webgpuFallbackHTML(probe) {
  ...
  return `
    <strong>Qwen3 1.7B runs on WebLLM, which needs WebGPU — and it isn't available in this browser.</strong>
    <p class="muted" style="margin:.4rem 0">${why}${
    probe.detail ? " (" + escapeHTML(probe.detail) + ")" : ""
  }
    WebLLM has <em>no WASM fallback</em>: a 1.7B model decoding token-by-token (and, in thinking mode,
    reasoning first) needs a real GPU, and the weights are a <strong>~1.1 GB</strong> download (cached
    after the first load) needing roughly 2 GB of GPU memory. So this page won't fake a reply. To run it
    for real:</p>
...
/* Qwen3 reasoning trace — the model's private "thinking" before it answers. */
...
.chip{font:inherit;font-size:.78rem;padding:.3rem .6rem;border-radius:999px;border:1px solid var(--border);background:var(--bg-raised);color:var(--color);cursor:pointer;min-block-size:2.2rem}

// Front-end helpers shared by every SCUNet denoising page. Keeps pages thin: owns the worker handshake
// (transferring ImageBitmaps so nothing is copied), turns files / gallery images into ImageBitmaps, can
// synthesise a NOISY input (additive Gaussian noise) so the demo can show the network cleaning a known
// noise, and renders the denoised result + an accessible before/after reveal slider. ALL inference AND
// the dense output composite live in worker.js (off the main thread, raw ONNX Runtime Web). Privacy by
// construction: the image and every denoised pixel never leave the device.

const WORKER_URL = "/web-ai-showcase/models/scunet-image-denoising/worker.js";
...
export class DenoiseEngine {
  ...
  /** Denoise an ImageBitmap (transferred → zero-copy). Returns
   * { denoisedBmp, noiseBmp, w, h, imgW, imgH, inHF, outHF, ms, infMs, device }. */
  denoise(bitmap, opts) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
...
// Synthesise a NOISY input with additive Gaussian noise (the classic image-denoising degradation).
// `sigma` is the noise standard deviation in 0–255 units (0 = untouched, ~50 = heavy grain). Honest by
// design — the user chooses the noise and watches SCUNet remove it; genuinely noisy photos (low-light
// grain, high ISO) can be uploaded instead. Applied on a bounded working copy so the grain is meaningful
// at the model's input scale.
const WORK = 512;
export async function addNoise(bitmap, { sigma = 25 } = {}) {
  ...
  if (sigma <= 0) return c.transferToImageBitmap();
  ...
  for (let i = 0; i < d.length; i += 4) {
    for (let k = 0; k < 3; k++) {
      ...
      d[i + k] = Math.max(0, Math.min(255, d[i + k] + n * sigma));
    }
  }
...
export class RevealCompare {
  ...
  _apply() {
    ...
    this.range.setAttribute("aria-valuetext", `${Math.round(pct)}% denoised`);
  }
...
/** Shared inline styles for the denoising widgets. Structural only — colours from design-system vars. */
export const DENOISE_CSS = `
...
.noise-canvas { inline-size: 100%; max-inline-size: 300px; block-size: auto; border-radius: 6px; border: 1px solid var(--border); display: block; }

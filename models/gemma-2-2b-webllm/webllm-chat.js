// Front-end helpers shared by every Gemma-2-2B (WebLLM) page: the worker handshake + streaming
...
const WORKER_URL = "/web-ai-showcase/models/gemma-2-2b-webllm/worker.js";
...
/**
 * Gemma 2's chat template has NO dedicated system role — only alternating user/model turns
 ...
 *  the ~1.6 GB download so a visitor knows what running it for real would cost. */
export function webgpuFallbackHTML(probe) {
  ...
  return `
    <strong>Gemma 2 2B runs on WebLLM, which needs WebGPU — and it isn't available in this browser.</strong>
    <p class="muted" style="margin:.4rem 0">${why}${
    probe.detail ? " (" + escapeHTML(probe.detail) + ")" : ""
  }
    WebLLM has <em>no WASM fallback</em>: a 2.6B model decoding token-by-token needs a real GPU, and the
    weights are a <strong>~1.6 GB</strong> download (cached after the first load). So this page won't
    fake a reply. To run it for real:</p>
    <ul class="muted" style="margin:.2rem 0">
      ...
      <li>The q4f16_1 build is fastest on GPUs that report the <code>shader-f16</code> feature (most modern ones), and needs roughly 2 GB of GPU memory.</li>
    </ul>`;
}

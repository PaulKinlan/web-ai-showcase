// Front-end helpers shared by every Real-ESRGAN super-resolution page. Keeps pages thin: owns the
// worker handshake (transferring ImageBitmaps so nothing is copied), turns files / gallery images into
// ImageBitmaps, can synthesise a low-resolution source (shrink to a thumbnail) so you can watch
// Real-ESRGAN enlarge it, and renders an accessible before/after reveal between a plain BICUBIC upscale
// and the Real-ESRGAN upscale (an honest like-for-like comparison, both at the output resolution). ALL
// inference AND the dense output composite live in worker.js (off the main thread, raw ONNX Runtime
// Web). Privacy by construction: the image and every upscaled pixel never leave the device.

const WORKER_URL = "/web-ai-showcase/models/real-esrgan-super-resolution/worker.js";
...
export class EsrganEngine {
  ...
  /** Upscale an ImageBitmap 4× (transferred → zero-copy). Returns
   * { esrBmp, bicBmp, deltaBmp, inW, inH, outW, outH, scale, sharpBic, sharpEsr, ms, infMs, device }. */
  upscale(bitmap, opts) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
...
// Synthesise a LOW-RESOLUTION source so the demo can show Real-ESRGAN enlarging it. Shrinks the image so
// its long side is `size` px (a small thumbnail). Optionally re-encodes as a low-quality JPEG to add the
// blocky compression artefacts Real-ESRGAN was trained to undo. Returns a fresh ImageBitmap. Honest by
// design — the user chooses how small/compressed the source is and watches it come back; real
// low-resolution images can be uploaded instead.
export async function shrink(bitmap, { size = 128, jpeg = 0 } = {}) {
  const long = Math.max(bitmap.width, bitmap.height);
  const s = Math.min(size / long, 1);
...
export class RevealCompare {
  ...
  _apply() {
    ...
    this.range.setAttribute("aria-valuetext", `${Math.round(pct)}% Real-ESRGAN`);
  }
...
/** Shared inline styles for the super-resolution widgets. Structural only — colours from design-system vars. */
export const ESRGAN_CSS = `
...
.fit-canvas { inline-size: 100%; block-size: auto; display: block; max-inline-size: 100%; border-radius: 6px; image-rendering: auto; }

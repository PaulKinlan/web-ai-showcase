// Front-end helpers for the German wav2vec2 ASR pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT language from every other ASR demo in this showcase: this is GERMAN — wav2vec2-base-10k-
// voxpopuli fine-tuned on German (OpenVoiceOS ONNX export), a Latin-script CTC recogniser over a
// 36-symbol vocab with the German diacritics (ä ö ü ß). NOT the wav2vec2-large-
// XLSR-53 model (no German XLSR-53 ONNX exists) — see the model page's at-a-glance. Because the only
// German ONNX export is an onnx-asr graph transformers.js can't load (model_type "wav2vec2-ctc"), the
// worker runs it directly on ONNX Runtime Web. The engine is the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the German alphabet.

const WORKER_URL = "/web-ai-showcase/models/german-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class GermanAsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (German word + start–end seconds). */

// Front-end helpers for the wav2vec2 pages. Keeps each page thin: worker handshake, audio decode to the
// 16 kHz mono Float32Array the model wants, a tiny mic recorder, waveform draw, and shared CSS.
// All inference lives in worker.js (off the main thread).
//
// wav2vec2 is a DIFFERENT architecture from Whisper/Moonshine/Distil-Whisper. Those are seq2seq: an
// encoder-decoder that AUTOREGRESSIVELY generates text tokens (and can hallucinate). wav2vec2 is a CTC
// model: the encoder emits, for every ~20 ms audio frame, a probability over a tiny character vocabulary
// (26 letters + apostrophe + a word-boundary "|" + a blank). The transcript is just the per-frame argmax
// with repeats and blanks collapsed — no decoder, no autoregression, no hallucination. This engine
// returns that per-frame strip so the page can VISUALISE the collapse, which is the whole point.

const WORKER_URL = "/web-ai-showcase/models/wav2vec2-asr/worker.js";
const TARGET_RATE = 16000;

export class Wav2VecEngine {
  constructor() {
   ...
   *  { text, strip:[{c,blank,boundary}], collapsed:[chars], frameMs, frames, ms, tokPerSec, audioSec, device }. */
 ...
 * emitted characters in the accent, a word boundary marked. This is the "argmax → collapse" view.
 * `strip` = [{ c, blank, boundary }]. Returns nothing; mutates `el`.
 */
...
/** wav2vec2 widget styles, injected once per page (keeps us on the shared design system). */
export const WAV2VEC_CSS = `
...
.ctc-collapsed { font-family:var(--font-mono); font-size:1.1rem; letter-spacing:.04em; margin:.4rem 0;
...
.align-word { display:inline-block; padding:.15rem .5rem; margin:.15rem; border-radius:999px;
  border:1px solid var(--border); background:var(--bg-raised); font-family:var(--font-mono); font-size:.82rem; }
.align-word .t { color:var(--muted); font-size:.72rem; }
`;

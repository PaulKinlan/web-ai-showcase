// Front-end helpers for the nanoLLaVA pages: the worker handshake, streaming plumbing, a real WebGPU
// probe (for the honest fallback), image helpers, and the widget CSS. Inference is in worker.js.

const WORKER_URL = "/web-ai-showcase/models/nanollava-vlm/worker.js";

export class NanoLlavaEngine {
  constructor() {
  ...
  generate(imageURL, prompt, { maxTokens = 200, system, onToken, onPrompt } = {}) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
      this._active = { id, onToken, onPrompt, resolve, reject };
      this.worker.postMessage({ type: "run", id, image: imageURL, prompt, system, maxTokens });
    });
  }
}

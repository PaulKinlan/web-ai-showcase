...
//   Stage 1 (retriever): Xenova/all-MiniLM-L6-v2 (feature-extraction) — fast bi-encoder embeddings.
//   Stage 2 (reranker):  Xenova/bge-reranker-base (cross-encoder) — precise relevance on the shortlist.
...
const EMBED_ID = "Xenova/all-MiniLM-L6-v2";
const RERANK_ID = "Xenova/bge-reranker-base";
...
async function ensureLoaded() {
  ...
  if (!embedPipe) {
    post({ type: "stage", stage: "Loading MiniLM retriever…" });
  ...
  if (!rerankPipe) {
    post({ type: "stage", stage: "Loading BGE reranker…" });

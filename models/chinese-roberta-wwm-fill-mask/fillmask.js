// Front-end helpers for the Chinese RoBERTa-wwm-ext fill-mask pages. Thin: owns the worker handshake +
// renderers. All inference (masked-LM logits, softmax, candidate scoring) lives in the workers, off the
// main thread.

export const MASK = "[MASK]";

const RWWM_WORKER_URL = "/web-ai-showcase/models/chinese-roberta-wwm-fill-mask/worker.js";
const BERT_WORKER_URL = "/web-ai-showcase/models/chinese-roberta-wwm-fill-mask/bert-worker.js";
...
export class ChineseRobertaFillMaskEngine extends FillMaskEngineBase {
  constructor() {
    super(RWWM_WORKER_URL);
  }
...
// Vanilla bert-base-chinese, loaded on demand for the head-to-head comparison.
export class ChineseBertFillMaskEngine extends FillMaskEngineBase {
  constructor() {
    super(BERT_WORKER_URL);
  }
}
 ...
 * `onPick` (optional) fires with the chosen character when a row is activated (keyboard + click).
 */
export function renderPredictions(container, predictions, onPick) {
  ...
  container.replaceChildren(...predictions.map((p, i) => {
    ...
    tok.className = "pred-tok pred-han";
...
/** The "see inside" character strip: every input token as a chip, so a visitor can SEE that this model
 *  tokenises one Hànzì at a time (no word segmentation). Special/mask tokens are marked. */
export function renderTokenStrip(container, tokens) {
  container.replaceChildren(...tokens.map((t) => {
    const chip = document.createElement("span");
    chip.className = "char-chip" + (t.isMask ? " is-mask" : "") +
      (t.isSpecial ? " is-special" : "");
    chip.textContent = t.isMask ? "▢" : t.ch;
    chip.title = t.isMask ? "[MASK] — the position being predicted" : `token id ${t.id}`;
    return chip;
  }));
}

/** Render a candidate-scoring probe (P(char) at the mask across a fixed set, each with an optional gloss). */
export function renderCandidates(container, scores, glosses) {
  ...
  container.replaceChildren(...scores.map((s) => {
    ...
    if (glosses && glosses[s.word]) {
      const g = document.createElement("span");
      g.className = "cand-gloss";
      g.textContent = glosses[s.word];
      w.append(" ", g);
    }
    const barWrap = document.createElement("span");
...
.mask-chip { font: inherit; font-size: .82rem; padding: .3rem .6rem; border-radius: 999px;
  border: 1px solid var(--border); background: var(--bg-raised); color: var(--color); cursor: pointer;
  display: inline-flex; align-items: center; gap: .35rem; min-block-size: 1.9rem; }
...
.pred-row { display: grid; grid-template-columns: 2.2rem 4rem 1fr 3.4rem 5.2rem; align-items: center;
...
.pred-han { font-size: 1.35rem; line-height: 1; }
...
@media (max-width: 560px) {
  .pred-row { grid-template-columns: 1.8rem 3rem 1fr 3rem; }
...
.filled-sentence { font-family: var(--font-display); font-size: 1.4rem; margin: .3rem 0 .2rem; line-height: 1.6; }
.filled-sentence .slot { color: var(--accent); font-weight: 700; border-bottom: 2px dotted var(--accent); }
.char-strip { display: flex; flex-wrap: wrap; gap: .3rem; margin-top: .5rem; }
.char-chip { display: inline-flex; align-items: center; justify-content: center; min-inline-size: 2rem;
  block-size: 2rem; font-size: 1.15rem; border-radius: 6px; border: 1px solid var(--border);
  background: var(--bg-raised); }
.char-chip.is-mask { border-color: var(--accent); color: var(--accent); font-weight: 700; }
.char-chip.is-special { color: var(--muted); font-size: .7rem; font-family: var(--font-mono); }
...
.cand-word { font-size: 1.1rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.cand-gloss { font-family: var(--font-mono); font-size: .72rem; color: var(--muted); }
...
.en-out { font-size: 1.05rem; margin: .3rem 0; }
`;

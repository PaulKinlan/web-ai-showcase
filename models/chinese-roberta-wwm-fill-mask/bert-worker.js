// Comparison worker: Google's vanilla bert-base-chinese, loaded on demand for the apples-to-apples
// head-to-head against chinese-roberta-wwm-ext on this page. Model: Xenova/bert-base-chinese
// (task: fill-mask), WASM, q8 (~103 MB). Same character-level 21,128-token vocabulary + [MASK] token as
// the RoBERTa-wwm model, so the SAME sentence and the SAME candidate set are directly comparable — the
// only difference is the training (vanilla MLM+NSP on Chinese Wikipedia vs WWM + RoBERTa-style + ext
// corpus). Inference runs off the main thread.
...
async function fill(id, text, topk) {
  ...
  const masks = positions.map((pos) => {
    ...
    return { pos, predictions: topkFromRow(row, vocab, topk || 8) };
  });
  post({
    type: "fill",
    id,

// Front-end helpers for the Baguettotron pages: the worker handshake, streaming plumbing, and the
// shared widget CSS. Inference lives in worker.js (Transformers.js text-generation). Every Baguettotron
// page imports this so the chat loop and "see inside" surface stay identical.
//
// Baguettotron is Pleias's small "encyclopedic" REASONING model (~321M params, 80 layers deep). Its
// ONNX build (onnx-community/Baguettotron-ONNX) exports as a `llama`-architecture decoder, so
// Transformers.js loads it through the standard text-generation pipeline. It reasons out loud inside a
// <think> block, then answers. The worker runs the q8 build on the universal WebAssembly/CPU path (the
// q4 block-quant build aborts on the WASM EP); WebGPU is an optional accelerator with a WASM fallback.

const WORKER_URL = "/web-ai-showcase/models/baguettotron/worker.js";

export class BaguettotronEngine {
  constructor() {

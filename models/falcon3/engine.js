// Front-end helpers for the Falcon3-1B-Instruct pages: the worker handshake, streaming plumbing, and
// the shared widget CSS. Inference lives in worker.js (Transformers.js text-generation). Every Falcon3
// page imports this so the chat loop and "see inside" surface stay identical.
//
// Falcon3-1B-Instruct is TII's Falcon3 family — a compact, strongly multilingual instruct LLM. The
// ONNX build (onnx-community/Falcon3-1B-Instruct) is a `llama`-architecture export (Falcon3 adopts the
// Llama decoder), so Transformers.js loads it through the standard text-generation pipeline. The worker
// probes for a real WebGPU adapter and runs on WebGPU (dtype q4f16) when present, else on the universal
// WebAssembly/CPU path (dtype q8) — the point of this ~1B model is that it runs on ANY device.

const WORKER_URL = "/web-ai-showcase/models/falcon3/worker.js";

export class Falcon3Engine {
  constructor() {

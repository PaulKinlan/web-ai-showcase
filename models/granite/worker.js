// IBM Granite 3.0 2B Instruct — text generation off the main thread via Transformers.js.
// Model: onnx-community/granite-3.0-2b-instruct (task: text-generation).
//
// Granite 3.0 (IBM) is an enterprise-tuned instruct family (Apache-2.0), built for business tasks:
// structured extraction, RAG, tool/function calling, summarisation. This ONNX build is a genuine
// `granite`-architecture export (GraniteForCausalLM), so the standard Transformers.js text-generation
// pipeline loads it directly. When a real WebGPU adapter is present the worker uses the recommended
// WebGPU path (dtype q4f16, onnx/model_q4f16.onnx, ~1.6 GB); otherwise it runs on the WebAssembly/CPU
// path (dtype q8, onnx/model_quantized.onnx, ~2.5 GB — a 2B model, slower on CPU). Either way the
// output is a real, on-device run — never a canned reply.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const MODEL_ID = "onnx-community/granite-3.0-2b-instruct";
let generator = null;
let mod = null;
let stopper = null;
let DEVICE = "wasm";
...
async function ensureLoaded() {
  ...
  DTYPE = picked.dtype;
  console.log(`[granite worker] loading ${MODEL_ID} on ${DEVICE} (${DTYPE})`);
  generator = await pipeline("text-generation", MODEL_ID, {
    device: DEVICE,
    dtype: DTYPE,
    progress_callback: (p) => post({ type: "progress", p }),
  });
  console.log(`[granite worker] ready on ${DEVICE}`);
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[granite worker] error", err);

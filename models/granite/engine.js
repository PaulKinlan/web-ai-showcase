// Front-end helpers for the IBM Granite 3.0 2B Instruct pages: the worker handshake, streaming
// plumbing, and the shared widget CSS. Inference lives in worker.js (Transformers.js text-generation).
// Every Granite page imports this so the chat loop and "see inside" surface stay identical.
//
// Granite 3.0 (IBM) is an enterprise-tuned instruct family — trained with a strong emphasis on
// business tasks (structured extraction, RAG, tool/function calling, summarisation) and released under
// Apache-2.0 for commercial use. The ONNX build (onnx-community/granite-3.0-2b-instruct) is a genuine
// `granite`-architecture export (GraniteForCausalLM), so Transformers.js loads it through the standard
// text-generation pipeline. The worker probes for a real WebGPU adapter and runs on WebGPU
// (dtype q4f16, the recommended enterprise workstation path) when present, else on the WebAssembly/CPU
// path (dtype q8). Either way the output is a real, on-device run — never a canned reply.

const WORKER_URL = "/web-ai-showcase/models/granite/worker.js";
...
export class GraniteEngine {
  ...
  /** Load the model. The worker picks WebGPU (q4f16) or WASM (q8). Resolves with the device string. */
  load(onProgress) {
...
.bubble { padding:.55rem .8rem; border-radius:12px; max-inline-size:85%; min-inline-size:0;
...
.chip { font-size:.8rem; padding:.5rem .8rem; min-block-size:40px; border:1px solid var(--border);
  border-radius:999px; background:var(--bg-raised); color:var(--color); cursor:pointer; }

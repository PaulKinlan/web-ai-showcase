// Shared front-end helpers for the OWLViT (v1) zero-shot detection pages. Keeps each page thin: it owns
// the worker handshake, turns files/samples into data URLs, draws the box overlay (coloured per text
// query), parses the query field, and summarises per-query scores. All inference is in worker.js.
//
// OWLViT is the ORIGINAL open-vocabulary detector (Minderer et al., 2022); OWLv2 is its self-trained
// successor. Same task + pipeline shape (zero-shot-object-detection), so the UI mirrors the OWLv2 page —
// the point of this demo is to run v1 honestly and contrast it with v2.

const WORKER_URL = "/web-ai-showcase/models/owlvit-detection/worker.js";

export class OwlvitEngine {
  constructor() {
...
/** Parse a comma-separated / newline query field into a clean, de-duped list of text prompts. */
...
/** Shared inline styles for the OWLViT widgets (canvas stage, dropzone, query chips, box table). */
export const OWLVIT_CSS = `
.dropzone {
...
.query-field textarea { inline-size: 100%; min-inline-size: 0; }
...
.chip {
  font: inherit; font-size: .78rem; padding: .3rem .6rem; border-radius: 999px;

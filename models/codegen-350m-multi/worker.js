// CodeGen-350M-multi worker — code generation off the main thread, WASM q8.
// Model: Xenova/codegen-350M-multi (task: text-generation).
//
// Salesforce's CodeGen is one of the first open autoregressive code language models: a 350M decoder
// trained on The Pile + BIGQUERY (public GitHub code), multi = multi-language (Python/Java/JS/C++). Given a
...
const MODEL_ID = "Xenova/codegen-350M-multi";
const DTYPE = "q8";
let generator = null;
let mod = null;
let device = "wasm";
...
async function generate(id, prompt, opts) {
  ...
  const streamer = new TextStreamer(generator.tokenizer, {
    ...
    // TextStreamer buffers decoded words, so a callback is not one token. Count the generated
    // token IDs (prompt excluded) and attach the running count to each visible chunk.

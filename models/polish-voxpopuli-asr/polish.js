// Front-end helpers for the Polish wav2vec2 ASR pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT language from every other ASR demo in this showcase: this is POLISH — wav2vec2-base-10k-
// voxpopuli fine-tuned on Polish (onnx-community ONNX mirror), a Latin-script CTC recogniser over a
// 41-symbol vocab with the Polish diacritics (ą ć ę ł ń ó ś ź ż). NOT the 315M XLSR-53 large model
// (no Polish XLSR-53 ONNX exists) — see the model page's at-a-glance. The engine is the same CTC shape
// (per-frame argmax → collapse), which lets the page visualise the collapse over the Polish alphabet.

const WORKER_URL = "/web-ai-showcase/models/polish-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class PolishAsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Polish word + start–end seconds). */

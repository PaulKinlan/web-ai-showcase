// MMS-TTS Gujarati (VITS) worker — end-to-end Gujarati speech synthesis off the main thread so the
...
// There is no separate vocoder: Gujarati text → language-specific token ids → (flows + duration) →
// 16 kHz waveform, all in one network. Nothing here is faked.
//
// This demo is the GUJARATI checkpoint (naklitechie/mms-tts-gu-ONNX) — a materially DISTINCT model from
// the built English/German/Spanish/French/Arabic/Vietnamese/Hindi/Tamil MMS-TTS demos: its own native
// Gujarati-script character vocabulary (60 symbols, is_uroman:false — native Gujarati, NOT romanised
// Latin, and a different script from Hindi's Devanagari and Tamil's Tamil script) and its own VITS
// weights, trained on Gujarati speech.
//
// PROVENANCE + TOKENISER NOTE (why this worker loads the model + tokenises directly instead of the
// pipeline): naklitechie/mms-tts-gu-ONNX is an ONNX export whose vocab.json matches the canonical
// facebook/mms-tts-guj, but the repo ships model.onnx at the repo root (not onnx/) and does NOT include a
// tokenizer.json, so the high-level pipeline("text-to-speech") cannot load it. Because this checkpoint is
// is_uroman:false (confirmed in tokenizer_config.json: is_uroman:false, language:"guj", add_blank:true),
...
// canonical vocab.json, and interleave the blank/pad token (id 0, "|") as add_blank requires. This manual
// tokeniser follows the SAME algorithm proven byte-identical to transformers.js's real VitsTokenizer on a
// control language in the Tamil demo (Xenova/mms-tts-hin: realLen==manualLen, identical id sequence). The
// VITS model itself runs via the real transformers.js AutoModel, so the waveform is genuine model output.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const DEFAULT_MODEL = "naklitechie/mms-tts-gu-ONNX";
...
async function ensureLoaded(modelId, notifyProgress) {
  ...
  state.Tensor = Tensor;
  // Canonical vocab (matches facebook/mms-tts-guj) for the deterministic character tokeniser.
...
async function synth(id, modelId, text) {
  ...
  if (inputIds.length <= 1) {
    post({ type: "error", id, message: "No Gujarati characters recognised in the input." });
    return;
  }

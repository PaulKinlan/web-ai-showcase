// Front-end helpers for the MMS-TTS Gujarati (VITS) pages. Thin: owns the worker handshake, WAV encoding
// and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: naklitechie/mms-tts-gu-ONNX — an ONNX export of the canonical Meta MMS GUJARATI VITS checkpoint
// (its vocab.json matches facebook/mms-tts-guj). ~109 MB fp32, 16 kHz mono output. This is a DISTINCT
// model from the built English/German/Spanish/French/Arabic/Vietnamese/Hindi/Tamil MMS-TTS demos: its own
// native Gujarati-script char vocab (60 symbols, is_uroman:false — a different script from Hindi's
// Devanagari and Tamil's Tamil script) + Gujarati-trained VITS weights. The worker reproduces the
// VitsTokenizer exactly (the same algorithm proven identical to the real tokenizer in the Tamil demo)
// because this export omits tokenizer.json.

const WORKER_URL = "/web-ai-showcase/models/mms-tts-gujarati/worker.js";
export const MODEL_ID = "naklitechie/mms-tts-gu-ONNX";

export class MmsGuEngine {
  constructor() {
  ...
  /** Synthesize Gujarati text → { audio: Float32Array, rate, ms, samples, device, inputIds } */
...
// A small set of Gujarati (Gujarati script) example lines that exercise vowels, matras and phrasing.
export const GUJARATI_SAMPLES = [
  {
    label: "પરિચય",
    text: "આ અવાજ સંપૂર્ણપણે તમારા બ્રાઉઝરમાં બને છે, કોઈ સર્વર વગર.",
  },
  {
    label: "નમસ્તે",
    text: "નમસ્તે! તમે કેમ છો? તમને મળીને આનંદ થયો.",
  },
  {
    label: "હવામાન",
    text: "આ સપ્તાહના અંતે હવામાન મોટે ભાગે સ્વચ્છ રહેશે, મહત્તમ તાપમાન બત્રીસ ડિગ્રી.",
  },
  {
    label: "સમાચાર",
    text: "સંશોધકોએ ભાષા મોડેલ સંપૂર્ણપણે બ્રાઉઝરમાં ચલાવવાની નવી રીત રજૂ કરી છે.",
  },
  {
    label: "જાહેરાત",
    text: "કૃપા કરીને ધ્યાન આપો: અમદાવાદ જતી ટ્રેન આજે પાંચમા પ્લેટફોર્મ પરથી ઉપડશે.",
  },
  {
    label: "કહેવત",
    text: "બોલે તેના બોર વેચાય; મહેનત કરે તે જ પામે.",
  },
];

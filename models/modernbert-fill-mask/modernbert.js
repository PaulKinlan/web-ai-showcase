// Front-end helpers for the ModernBERT fill-mask pages. Thin: owns the worker handshake + renderers.
// All inference (masked-LM logits, softmax, candidate scoring) lives in worker.js, off the main
// thread, so typing/long-context runs never jank the control UI.

const WORKER_URL = "/web-ai-showcase/models/modernbert-fill-mask/worker.js";
export const MASK = "[MASK]";

export class ModernBertEngine {
  constructor() {
  ...
  /** Fill one text → { text, masks:[{pos, predictions:[{token,logit,prob}]}], maskCount, tokenCount, ms, device } */
  fill(text, topk = 8) {
    return this._call({ type: "fill", text, topk });
  }

  /** Fill a batch of texts → { results:[{text,masks,maskCount,tokenCount}], ms, device } */
...
/** Shannon entropy (bits) approximated from the returned top-k probs. */
...
textarea.mask-input { resize: vertical; min-block-size: 4.5rem; line-height: 1.5; }
...
@media (max-width: 560px) {
  .pred-row { grid-template-columns: 1.8rem 5.5rem 1fr 3rem; }
...
.arch-table { inline-size: 100%; border-collapse: collapse; font-size: .82rem; margin-top: .5rem; }
.arch-table th, .arch-table td { text-align: left; padding: .35rem .55rem; border-bottom: 1px solid var(--border); vertical-align: top; }
.arch-table th { color: var(--muted); font-weight: 600; font-family: var(--font-mono); }
.arch-table td:first-child { font-family: var(--font-mono); color: var(--muted); white-space: nowrap; }
.arch-table .now { color: var(--accent); font-weight: 600; }
.arch-table caption { text-align: left; color: var(--muted); font-size: .78rem; margin-bottom: .3rem; }
...
.tok-meter { display:flex; align-items:center; gap:.5rem; flex-wrap:wrap; font-family:var(--font-mono); font-size:.78rem; color:var(--muted); margin:.3rem 0; }
.tok-meter .track { flex:1 1 160px; max-inline-size:320px; block-size:.55rem; border-radius:999px; background:var(--bg-raised); border:1px solid var(--border); overflow:hidden; }
.tok-meter .track > span { display:block; block-size:100%; background:var(--accent); }
`;

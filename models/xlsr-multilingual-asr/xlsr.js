// Front-end helpers for the wav2vec2-XLSR-53 (Russian) pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT from the English wav2vec2 demo: this model is XLSR-53 — cross-lingual pre-training over 53
// languages, fine-tuned for RUSSIAN — so its output is Cyrillic and it is language-specialised. The
// engine is otherwise the same CTC shape (per-frame argmax → collapse), which lets the page visualise
// the collapse over a Cyrillic alphabet.

const WORKER_URL = "/web-ai-showcase/models/xlsr-multilingual-asr/worker.js";
const TARGET_RATE = 16000;

export class XlsrEngine {
  constructor() {
 ...
 * emitted Cyrillic characters in the accent, a word boundary marked. `strip` = [{ c, blank, boundary }].
 */
...
/** Render per-word forced-alignment chips (Cyrillic word + start–end seconds). */

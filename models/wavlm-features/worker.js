// WavLM feature-extraction worker — ALL inference AND the dense heatmap composite run off the main
// thread (invariant 15: measure, don't infer; keep INP low; transfer RGBA, don't clone).
//
// WavLM is a SELF-SUPERVISED SPEECH representation model — a DIFFERENT thing from the ASR models in
// this showcase (wav2vec2 CTC, Whisper seq2seq). Those emit TEXT. WavLM emits, for every ~20 ms audio
// frame, a 768-dim CONTEXTUAL EMBEDDING vector (`last_hidden_state`, shape [1, T, 768]) — no letters,
// no words, just a learned representation of what the audio "is" at that moment. It was pretrained on
// 94k h of speech with masked-speech-prediction PLUS a denoising/overlapped-speech objective and a
// gated relative-position bias, which is why its frame embeddings carry speaker + phonetic + prosodic
// structure you can compare, cluster, and search. We call the model DIRECTLY (AutoModel, not a
// pipeline) so we can return the raw frames for the heatmap and do pooling/cosine ourselves.
//
// Model: Xenova/wavlm-base-plus (microsoft/wavlm-base-plus), q8 ONNX, WASM (WebGPU when an adapter
// exists). Transformers.js via the SHARED CDN url from lib/webai.js.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const MODEL = "Xenova/wavlm-base-plus";
const SR = 16000;
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[wavlm worker]", err);

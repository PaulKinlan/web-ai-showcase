// Front-end helpers for the MMS-TTS Urdu (VITS) pages. Thin: owns the worker handshake, WAV encoding
// and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: naklitechie/mms-tts-ur-ONNX — a faithful ONNX export of the canonical Meta MMS URDU VITS
// checkpoint (its vocab.json is byte-identical to facebook/mms-tts-urd-script_arabic). ~114 MB fp32,
// 16 kHz mono output. This is a DISTINCT model from the built MMS-TTS demos: its own native
// Arabic/Nastaliq-script char vocab (58 symbols, is_uroman:false — a right-to-left Perso-Arabic script,
// different again from Devanagari, Tamil and Malayalam) + Urdu-trained VITS weights. The worker
// reproduces the VitsTokenizer exactly (verified identical to the real tokenizer) because this export
// omits tokenizer.json.
//
// RTL: Urdu is written right-to-left in the Perso-Arabic (Nastaliq) script. The pages mark all Urdu
// content with lang="ur" dir="rtl" so the browser shapes and orders the script correctly; the layout
// itself uses logical properties so it mirrors cleanly.

const WORKER_URL = "/web-ai-showcase/models/mms-tts-urdu/worker.js";
export const MODEL_ID = "naklitechie/mms-tts-ur-ONNX";

export class MmsUrEngine {
  constructor() {
  ...
  /** Synthesize Urdu text → { audio: Float32Array, rate, ms, samples, device, inputIds } */
...
// A small set of Urdu (Perso-Arabic / Nastaliq script, RTL) example lines.
export const URDU_SAMPLES = [
  {
    label: "تعارف",
    text: "یہ آواز مکمل طور پر آپ کے براؤزر میں، کسی سرور کے بغیر بنائی جاتی ہے۔",
  },
  {
    label: "سلام",
    text: "السلام علیکم! آپ کیسے ہیں؟ آپ سے مل کر خوشی ہوئی۔",
  },
  {
    label: "موسم",
    text: "اس ہفتے کے آخر میں موسم زیادہ تر صاف رہے گا، زیادہ سے زیادہ درجہ حرارت بتیس ڈگری۔",
  },
  {
    label: "خبر",
    text: "محققین نے زبان کے ماڈل کو مکمل طور پر براؤزر میں چلانے کا نیا طریقہ متعارف کرایا ہے۔",
  },
  {
    label: "اعلان",
    text: "براہ کرم توجہ دیں: لاہور جانے والی ٹرین آج پانچویں پلیٹ فارم سے روانہ ہوگی۔",
  },
  {
    label: "مقولہ",
    text: "علم بانٹنے سے بڑھتا ہے، اور سیکھنے کا سفر کبھی ختم نہیں ہوتا۔",
  },
];
...
// Note the RTL affordances: Urdu textareas/paragraphs carry dir="rtl" in the markup; the layout uses
// logical properties so flex/grid mirror correctly.
export const MMS_UR_CSS = `
...
textarea.tts[dir="rtl"], p[dir="rtl"] { text-align: start; }

// Front-end helpers for the MMS-TTS Arabic (VITS) pages. Thin: owns the worker handshake, WAV encoding
// and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: Xenova/mms-tts-ara — the ARABIC VITS checkpoint from Meta's MMS family (ONNX q8, ~38 MB,
// 16 kHz mono output). This is a DISTINCT model from the built English/German/Spanish MMS-TTS demos:
// its own Arabic character vocabulary + Arabic-trained VITS weights, so it speaks Arabic orthography
// (the Arabic script, right-to-left) natively. Arabic text is rendered RTL in the UI (dir="rtl").

const WORKER_URL = "/web-ai-showcase/models/mms-tts-arabic/worker.js";
export const MODEL_ID = "Xenova/mms-tts-ara";

export class MmsAraEngine {
  constructor() {
  ...
  /** Synthesize Arabic text → { audio: Float32Array, rate, ms, samples, device } */
...
// A small set of Arabic example lines (with an English gloss for the picker). All Modern Standard Arabic.
export const ARABIC_SAMPLES = [
  { label: "ترحيب", gloss: "Welcome", text: "مرحبا بكم في هذا العرض التوضيحي الذي يعمل بالكامل داخل متصفحكم." },
  { label: "تحية", gloss: "Greeting", text: "السلام عليكم ورحمة الله وبركاته، أهلا وسهلا بكم." },
  { label: "الطقس", gloss: "Weather", text: "من المتوقع أن يكون الطقس مشمسا غدا مع درجات حرارة تصل إلى ثلاثين درجة." },
  { label: "إعلان", gloss: "Announcement", text: "نعتذر عن التأخير. سيغادر القطار المتجه إلى القاهرة من الرصيف الثالث." },
  { label: "حكمة", gloss: "Proverb", text: "العلم نور والجهل ظلام، فاطلبوا العلم من المهد إلى اللحد." },
  { label: "أرقام", gloss: "Numbers", text: "واحد، اثنان، ثلاثة، أربعة، خمسة، ستة، سبعة، ثمانية، تسعة، عشرة." },
];
...
/* Arabic input is right-to-left; the RTL class flips direction + alignment for the script. */
textarea.rtl, .rtl { direction: rtl; text-align: right; }
...
.chip { font: inherit; font-size: .8rem; padding: .35rem .7rem; border-radius: 999px; min-block-size: 2.4rem;
  border: 1px solid var(--border); background: var(--bg-raised); color: var(--color); cursor: pointer; display: inline-flex; align-items: center; gap: .4rem; }
.chip:hover, .chip:focus-visible { border-color: var(--accent); }
.chip .chip-ar { font-size: .95rem; }
.chip .chip-gloss { font-family: var(--font-mono); color: var(--muted); font-size: .72rem; }
...
.inside-table th, .inside-table td { text-align: start; padding: .3rem .5rem; border-bottom: 1px solid var(--border); overflow-wrap: anywhere; }
.inside-table.glance th { inline-size: 34%; color: var(--muted); }

// Front-end helpers for the Slovak wav2vec2 ASR pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT language from every other ASR demo in this showcase: this is SLOVAK — wav2vec2-base-10k-
// voxpopuli fine-tuned on Slovak (OpenVoiceOS ONNX export), a Latin-script CTC recogniser over a
// 48-symbol vocab with the Slovak diacritics (á ä č ď é í ĺ ľ ň ó ô ŕ š ť ú ý ž). NOT the wav2vec2-large-
// XLSR-53 model (no Slovak XLSR-53 ONNX exists) — see the model page's at-a-glance. Because the only
// Slovak ONNX export is an onnx-asr graph transformers.js can't load (model_type "wav2vec2-ctc"), the
// worker runs it directly on ONNX Runtime Web. The engine is the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the Slovak alphabet.

const WORKER_URL = "/web-ai-showcase/models/slovak-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class SlovakAsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Slovak word + start–end seconds). */

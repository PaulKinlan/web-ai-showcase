// Front-end helpers for the MMS-TTS Russian (VITS) pages. Thin: owns the worker handshake, WAV encoding
// and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: Xenova/mms-tts-rus — the RUSSIAN VITS checkpoint from Meta's MMS family (ONNX q8, ~38 MB,
// 16 kHz mono output). This is a DISTINCT model from the built English/German/Spanish/French/Arabic/
// Vietnamese/Hindi and the Indic MMS-TTS demos: its own native CYRILLIC character vocabulary (44 symbols,
// is_uroman:false — native Cyrillic, NOT romanised Latin, a distinct non-Indic, non-Latin script) +
// Russian-trained VITS weights, so it speaks Cyrillic orthography (а/б/в, soft/hard signs ь/ъ, ё/й)
// natively. This Xenova export ships the full tokenizer (tokenizer.json), so it loads with the plain
// text-to-speech pipeline() — no manual tokeniser needed.

const WORKER_URL = "/web-ai-showcase/models/mms-tts-russian/worker.js";
export const MODEL_ID = "Xenova/mms-tts-rus";

export class MmsRuEngine {
  constructor() {
  ...
  /** Synthesize Russian text → { audio: Float32Array, rate, ms, samples, device } */
...
// A small set of Russian (Cyrillic) example lines that exercise soft/hard signs, ё/й and phrasing.
export const RUSSIAN_SAMPLES = [
  {
    label: "Вступление",
    text: "Этот голос полностью создаётся в вашем браузере, без какого-либо сервера.",
  },
  {
    label: "Приветствие",
    text: "Здравствуйте! Как ваши дела? Очень приятно с вами познакомиться.",
  },
  {
    label: "Погода",
    text:
      "В эти выходные погода будет в основном ясной, максимальная температура около тридцати двух градусов.",
  },
  {
    label: "Новости",
    text:
      "Исследователи представили новый способ полностью запускать языковую модель прямо в браузере.",
  },
  {
    label: "Объявление",
    text: "Пожалуйста, внимание: поезд до Москвы сегодня отправляется с пятой платформы.",
  },
  {
    label: "Пословица",
    text: "Знание, которым делишься, только растёт; путь учения никогда не заканчивается.",
  },
];

// Shared front-end helpers for the I-JEPA image-feature pages. Keeps each page thin: worker handshake,
// light client-side maths (cosine, similarity matrix, nearest, clustering, patch correspondence), the
// renderers (thumbnail matrix, patch-feature heatmap, patch-NORM heatmap, dense correspondence field),
// image loading (samples + upload → data URL), and the injected CSS. All model inference lives in
// worker.js (off the main thread).
//
// I-JEPA emits a plain patch-token grid (no [CLS], no register tokens). The global descriptor is the
// MEAN of the patch tokens, L2-normalized. Cosine similarity between two descriptors = image similarity;
// per-patch cosine-to-descriptor + per-patch norm are the "see inside" surfaces; unit patch embeddings
// drive dense patch-to-patch correspondence between two images.

const WORKER_URL = "/web-ai-showcase/models/ijepa-features/worker.js";

export class IJepaEngine {
  constructor() {
   ...
   * → { clsEmb (=mean descriptor), dim, clsPreNorm, patchSims, patchNorms, gridSize, numPatches, ms,
   *     device, [patchEmbs], [patchDim] }
   */
  embed(imageURL, opts = {}) {
...
/**
 * Render the patch-feature heatmap: a gridSize×gridSize grid, overlaid semi-transparently over the
 * source thumbnail. `mode` = "sim" (cosine to the global mean descriptor) or "norm" (per-patch L2 norm).
 */
export function renderPatchGrid(container, values, gridSize, mode = "sim") {
  const lo = Math.min(...values), hi = Math.max(...values);
  const span = hi - lo || 1;
  const label = mode === "norm" ? "patch token L2 norm" : "cosine to global descriptor";

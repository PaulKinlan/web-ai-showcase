// Front-end helpers for the XLM-RoBERTa fill-mask pages. Thin: owns the worker handshake + renderers.
// All inference (masked-LM logits, softmax, candidate scoring) lives in worker.js, off the main thread.

const WORKER_URL = "/web-ai-showcase/models/xlm-roberta-fill-mask/worker.js";
export const MASK = "<mask>";

export class XlmrFillMaskEngine {
  constructor() {
...
.mask-chip .flag { font-size: .95rem; line-height: 1; }
.lang-tag { font-family: var(--font-mono); font-size: .72rem; color: var(--muted); }
...
.lang-badge { display: inline-flex; align-items: center; gap: .35rem; padding: .15rem .55rem;
  border-radius: 999px; border: 1px solid var(--border); background: var(--bg-raised); font-size: .8rem; }
.lang-badge b { font-family: var(--font-mono); }
`;

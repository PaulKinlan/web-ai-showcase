// Front-end helpers for the BERTimbau (Portuguese BERT) fill-mask pages. Thin: owns the worker
// handshake + renderers. All inference (masked-LM logits, softmax, candidate scoring, tokenisation)
// lives in the workers, off the main thread.

export const MASK = "[MASK]";

const PT_WORKER_URL = "/web-ai-showcase/models/portuguese-bert-fill-mask/worker.js";
const MBERT_WORKER_URL = "/web-ai-showcase/models/portuguese-bert-fill-mask/mbert-worker.js";
...
export class PortugueseFillMaskEngine extends FillMaskEngineBase {
  constructor() {
    super(PT_WORKER_URL);
  }
...
/**
 * Render the WordPiece token strip — the "see inside" surface that shows how Portuguese splits into
 * subwords. `tokens` come from the worker's tokenStrip().
 */
export function renderTokenStrip(container, tokens) {
  container.replaceChildren(...tokens.map((t) => {
    ...
    if (t.cont) chip.title = "WordPiece continuation (##) — part of a split Portuguese word";
    return chip;
  }));
}

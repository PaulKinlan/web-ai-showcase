// BERTimbau (Portuguese BERT) fill-mask worker — inference off the main thread so the control UI stays
// responsive. Model: luiseduardobrito/bert-base-portuguese-cased-onnx (task: fill-mask / masked
// language modelling), WASM, fp32 (the repo ships only onnx/model.onnx — there is no quantized build,
// so we run the full-precision graph; the model-loader states the real ~528 MB download honestly).
//
// This is BERTimbau — a BERT-base pre-trained ONLY on Portuguese (the brWaC Brazilian-Portuguese web
// corpus, by neuralmind). Its 29,794-token CASED WordPiece vocabulary is Portuguese-native: accented
// vowels (á é í ó ú â ê ô ã õ), the cedilla (ç), Portuguese casing, and the Brazilian everyday words a
// multilingual model can't afford to keep whole (ônibus, feijoada, saudade). No language flag, no
// shared multilingual budget. Its mask token is "[MASK]" (BERT/WordPiece style), NOT "<mask>".
//
...
// the raw pre-softmax scores over the whole 29,794-token Portuguese vocabulary — not just the top-k a
...
//                     plus the WordPiece token strip (shows how Portuguese words are split into pieces).
...
//                     (compare P(word_A) vs P(word_B) at the mask across Portuguese templates).

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const MODEL_ID = "luiseduardobrito/bert-base-portuguese-cased-onnx";
let tokenizer = null;
let model = null;
const device = "wasm";
...
async function ensureLoaded() {
  ...
  model = await AutoModelForMaskedLM.from_pretrained(MODEL_ID, {
    dtype: "fp32", // the ONNX repo ships only the full-precision graph (no *_quantized.onnx)
    device: "wasm",
    progress_callback: (p) => post({ type: "progress", p }),
  });
...
// the token strip below keeps the "##" so you can SEE where a Portuguese word was split.
...
// without sorting the whole 29k vocab: maintain a small ordered buffer (same approach as the BERT page).
...
// continuations. This is what makes Portuguese subword splitting visible in the "see inside" surface.
...
// Leading-subword id for a candidate word. Multi-token candidates (some Portuguese words are multi-piece)

// Front-end helpers for the AST (audio-classification) pages. Keeps each page thin: it owns the
// worker handshake, decodes/records audio into the 16 kHz mono Float32Array the model wants, draws the
// waveform + the log-mel spectrogram heatmap, and renders the top-label score bars. All inference lives
// in worker.js (off the main thread).

const WORKER_URL = "/web-ai-showcase/models/ast-audio-classification/worker.js";
...
export class LiveListener {
  ...
  stop() {
    this.running = false;
    try { this._node && (this._node.onaudioprocess = null); } catch {}
    try { this._src?.disconnect(); this._node?.disconnect(); } catch {}

// MMS-TTS Romanian (VITS) worker — end-to-end Romanian speech synthesis off the main thread so the control
...
// is no separate vocoder: Romanian text → language-specific token ids → (flows + duration) → 16 kHz
// waveform, all in one network. Nothing here is faked.
//
// This demo is the ROMANIAN checkpoint (Xenova/mms-tts-ron) — a materially DISTINCT model from the built
// English MMS-TTS demo: its own Romanian character vocabulary and its own VITS weights, trained on Romanian
// speech, so it renders Romanian orthography (accented vowels, ç, the ã/õ nasals and Romanian phonotactics) that the English
// checkpoint cannot. The default modelId is the Romanian one; the worker still accepts any mms-tts-<iso>
// id so a page can compare a second language if it wants.
//
// Because the duration predictor is stochastic, generating the same Romanian text twice yields DIFFERENT
// timing/prosody — that variation is real and is what the "stochastic prosody" surface visualises.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const DEFAULT_MODEL = "Xenova/mms-tts-ron";
...
async function getPipeline(modelId, notifyProgress) {
  ...
  // The Romanian family always speaks Xenova/mms-tts-ron; a caller-provided alternate id is
  // honoured via the cache key but the primary stage is the literal checkpoint.
  const tts = await pipeline("text-to-speech", "Xenova/mms-tts-ron", {
    dtype: "q8",
    device: "wasm",

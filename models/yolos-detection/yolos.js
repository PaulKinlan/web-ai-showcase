// Front-end helpers shared by every YOLOS page. Keeps each page thin: it owns the worker handshake,
// turns files/samples into data URLs, draws the box overlay on a canvas, and renders the box table.
// All inference lives in worker.js (off the main thread), so the control UI stays responsive.
//
// Model: Xenova/yolos-tiny (task: object-detection). YOLOS is a plain Vision Transformer with a tiny
// detection head bolted on — a very different architecture from DETR's ResNet-50 + encoder/decoder.

const WORKER_URL = "/web-ai-showcase/models/yolos-detection/worker.js";

export class YolosEngine {
  constructor() {
...
/** Fetch a same-origin sample image and turn it into a data URL (so the worker can decode it). */
...
/** Deterministic colour per class label so the same object type keeps its colour across frames. */
 ...
 * CSS scales the canvas down responsively, so boxes stay pixel-accurate. Boxes are in the
...
/** Shared inline styles for the YOLOS-specific widgets (canvas stage, dropzone, box table). */
export const YOLOS_CSS = `
.dropzone {
...
.compare-grid { display: grid; grid-template-columns: 1fr auto auto; gap: .1rem .9rem; font-size: .82rem; margin-top: .5rem; }
.compare-grid > div { padding: .3rem 0; border-bottom: 1px solid var(--border); }
.compare-grid .h { color: var(--muted); font-weight: 600; font-family: var(--font-body); }
.compare-grid .v { font-family: var(--font-mono); text-align: right; }
...
.big-count { font-family: var(--font-display); font-size: 2.4rem; line-height: 1.1; margin: .1rem 0; }

// Front-end helpers for the Jina reranker (tiny) pages. Owns the worker handshake and the renderers
// (ranked passage list, before/after reordering, cross-encoder-vs-lexical contrast). All inference
// lives in worker.js (off the main thread). This model's story is SPEED — the renderers surface real
// latency, per-pair time and token throughput, never a claim.

const WORKER_URL = "/web-ai-showcase/models/jina-reranker/worker.js";
...
export class RerankEngine {
  ...
  /** Rerank → { query, results:[…original order], ranked:[…by logit], ms, perPair, totalTokens, pairs, device }. */
...
/** Render the reranked passages (`ranked` sorted by cross-encoder logit, descending). */
...
/** BEFORE (retrieval order) vs AFTER (cross-encoder reranked), side by side. */
...
/** Cross-encoder rank vs naive lexical-overlap rank. */
...
.speed-tiles { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: .6rem; margin-top: .6rem; }
.speed-tile { border: 1px solid var(--border); border-radius: 8px; background: var(--bg-raised); padding: .6rem .7rem; }
.speed-tile .n { font-family: var(--font-display); font-size: 1.5rem; color: var(--color); display: block; }
.speed-tile .l { font-family: var(--font-mono); font-size: .72rem; color: var(--muted); }

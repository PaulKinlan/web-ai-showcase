// wav2vec2 (Czech) CTC worker — ALL inference off the main thread.
//
// DISTINCT model + language from every other ASR demo in this showcase. This is a CZECH speech
// recogniser: facebook/wav2vec2-base-10k-voxpopuli fine-tuned on Czech, exported to ONNX by the
// OpenVoiceOS project. All the built ASR demos (English wav2vec2, Russian/French/Spanish/Italian/
// Portuguese/Chinese/Japanese/Korean/Thai XLSR + Finnish/Polish VoxPopuli) are CTC too, but none is Czech.
//
// HONEST NOTE ON THE RUNTIME. Unlike the Polish/Finnish VoxPopuli demos (onnx-community mirrors that
// load straight through transformers.js as AutoModelForCTC), the ONLY browser ONNX export of the Czech
// VoxPopuli checkpoint is OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-cs-onnx, which is packaged for the
...
// in-browser run of the genuine Czech Meta-AI checkpoint — not a relabel of another language and not a fake.
//
...
// headless Chrome (ORT Web 1.20.1, WASM EP) to produce real log-probs [1, frames, 46] and a coherent Czech
...
const MODEL_ID = "OpenVoiceOS/wav2vec2-base-10k-voxpopuli-ft-cs-onnx";
const MODEL_BASE = `https://huggingface.co/${MODEL_ID}/resolve/main/`;
const VOCAB_URL = "/web-ai-showcase/models/czech-voxpopuli-asr/vocab.txt";
...
self.addEventListener("message", async (e) => {
  ...
  try {
    ...
    console.error("[wav2vec2-cs worker]", err);

// Front-end helpers for the Czech wav2vec2 ASR pages. Keeps each page thin: worker handshake,
...
//
// DISTINCT language from every other ASR demo in this showcase: this is CZECH — wav2vec2-base-10k-
// voxpopuli fine-tuned on Czech (OpenVoiceOS ONNX export), a Latin-script CTC recogniser over a
// 46-symbol vocab with the Czech diacritics (á č ď é ě í ň ó ř š ť ú ů ý ž). NOT the wav2vec2-large-
// XLSR-53 model (no Czech XLSR-53 ONNX exists) — see the model page's at-a-glance. Because the only
// Czech ONNX export is an onnx-asr graph transformers.js can't load (model_type "wav2vec2-ctc"), the
// worker runs it directly on ONNX Runtime Web. The engine is the same CTC shape (per-frame argmax →
// collapse), which lets the page visualise the collapse over the Czech alphabet.

const WORKER_URL = "/web-ai-showcase/models/czech-voxpopuli-asr/worker.js";
const TARGET_RATE = 16000;

export class CzechAsrEngine {
  constructor() {
...
/** Render per-word forced-alignment chips (Czech word + start–end seconds). */

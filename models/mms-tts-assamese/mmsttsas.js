// Front-end helpers for the MMS-TTS Assamese (VITS) pages. Thin: owns the worker handshake, WAV encoding
// and the waveform canvas. All inference runs in worker.js, off the main thread.
//
// Model: naklitechie/mms-tts-as-ONNX — a faithful ONNX export of the canonical Meta MMS ASSAMESE VITS
// checkpoint (its vocab.json is byte-identical to facebook/mms-tts-asm). ~114 MB fp32, 16 kHz mono
// output. This is a DISTINCT model from the built MMS-TTS demos: its own native Assamese (Eastern
// Nagari) char vocab (66 symbols, is_uroman:false) + Assamese-trained VITS weights. Assamese shares the
// Bengali-Assamese (Eastern Nagari) script but is a SEPARATE Assamese checkpoint with its own vocab and
// the characteristic Assamese letters ৰ (ra) and ৱ (wa) that distinguish it from Bengali. The worker
// reproduces the VitsTokenizer exactly (verified identical to the real tokenizer) because this export
// omits tokenizer.json.

const WORKER_URL = "/web-ai-showcase/models/mms-tts-assamese/worker.js";
export const MODEL_ID = "naklitechie/mms-tts-as-ONNX";

export class MmsAsEngine {
  constructor() {
  ...
  /** Synthesize Assamese text → { audio: Float32Array, rate, ms, samples, device, inputIds } */
...
// A small set of Assamese (Eastern Nagari script) example lines that exercise the characteristic
// Assamese ৰ/ৱ, vowel signs and conjuncts.
export const ASSAMESE_SAMPLES = [
  {
    label: "পৰিচয়",
    text: "এই কণ্ঠস্বৰ সম্পূৰ্ণৰূপে আপোনাৰ ব্ৰাউজাৰত, কোনো ছাৰ্ভাৰ অবিহনে সৃষ্টি হয়।",
  },
  {
    label: "নমস্কাৰ",
    text: "নমস্কাৰ! আপুনি কেনে আছে? আপোনাক লগ পাই ভাল লাগিল।",
  },
  {
    label: "বতৰ",
    text: "এই সপ্তাহৰ শেষত বতৰ বেছিভাগ সময় পৰিষ্কাৰ থাকিব, সৰ্বাধিক উষ্ণতা বত্ৰিশ ডিগ্ৰী।",
  },
  {
    label: "বাতৰি",
    text: "গৱেষকসকলে ভাষা মডেলক সম্পূৰ্ণৰূপে ব্ৰাউজাৰত চলোৱাৰ এক নতুন উপায় উপস্থাপন কৰিছে।",
  },
  {
    label: "ঘোষণা",
    text: "অনুগ্ৰহ কৰি মনোযোগ দিয়ক: গুৱাহাটীলৈ যোৱা ৰেলগাড়ীখন আজি পাঁচ নম্বৰ প্লেটফৰ্মৰ পৰা ৰাওনা হ'ব।",
  },
  {
    label: "প্ৰবাদ",
    text: "জ্ঞান ভাগ কৰিলে বাঢ়ে; শিকাৰ যাত্ৰা কেতিয়াও শেষ নহয়।",
  },
];

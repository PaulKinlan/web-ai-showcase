// Shared front-end helpers for the SmolVLM pages: the worker handshake, streaming plumbing,
// a real WebGPU probe (used for the honest fallback), and the widget CSS. Inference is in worker.js.

const WORKER_URL = "/web-ai-showcase/models/smolvlm-vision-language/worker.js";

export class SmolVLMEngine {
  constructor() {
...
export function webgpuFallbackHTML(probe) {
  ...
  return `
    <strong>SmolVLM needs WebGPU — and it isn't available in this browser.</strong>
    <p class="muted" style="margin:.4rem 0">${why}${
    probe.detail ? " (" + escapeHTML(probe.detail) + ")" : ""
  }
    A 256M-parameter VLM in 4-bit still needs a GPU to run at a usable speed, so this page won't fake a
    result. To try it for real:</p>

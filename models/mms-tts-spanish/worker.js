// MMS-TTS Spanish (VITS) worker — end-to-end Spanish speech synthesis off the main thread so the control
...
// is no separate vocoder: Spanish text → language-specific token ids → (flows + duration) → 16 kHz
// waveform, all in one network. Nothing here is faked.
//
// This demo is the SPANISH checkpoint (Xenova/mms-tts-spa) — a materially DISTINCT model from the built
// English (Xenova/mms-tts-eng) and German (Xenova/mms-tts-deu) MMS-TTS demos: its own Spanish character
// vocabulary and its own VITS weights, trained on Spanish speech, so it renders Spanish orthography
// (ñ, the accented vowels á/é/í/ó/ú, ü) and Spanish phonotactics the English/German checkpoints cannot.
// The default modelId is the Spanish one; the worker still accepts any mms-tts-<iso> id.
//
// Because the duration predictor is stochastic, generating the same Spanish text twice yields DIFFERENT
// timing/prosody — that variation is real and is what the "stochastic prosody" surface visualises.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const DEFAULT_MODEL = "Xenova/mms-tts-spa";

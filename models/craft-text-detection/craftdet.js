// Front-end helpers shared by every CRAFT text-detection page. Keeps pages thin: owns the worker
// handshake (transferring ImageBitmaps so nothing is copied), turns files / samples / camera frames
// into ImageBitmaps, and renders the detected word polygons + the region and affinity heatmaps. ALL
// inference and the word-box decode live in worker.js (off the main thread, raw ONNX Runtime Web).
// Privacy by construction: the image, the heatmaps, and the boxes never leave the device. This LOCATES
// text regions — it does not READ them (that's OCR; see the multi-model demo).

const WORKER_URL = "/web-ai-showcase/models/craft-text-detection/worker.js";
...
export class CraftEngine {
   ...
   * { regions, heat, heatAff, mapW, mapH, imgW, imgH, ms, infMs, device }. */
  detect(bitmap, opts) {
    const id = ++this._id;
    return new Promise((resolve, reject) => {
...
/** Draw detected word polygons (in image pixels) onto a canvas already showing the image at `scale`.
 * Oriented polygons hug slanted words; each is numbered so the region list can reference it. */
...
/** Paint a raw score map (Uint8, mapW×mapH) into a canvas as an indigo heatmap — the model's actual
 * per-pixel belief, before the box decode. Used for both the REGION and AFFINITY maps. */
...
/** Crop a single detected region out of a source bitmap into a canvas, de-rotating by the region angle
 * so the word sits upright — the tight, aligned crop an OCR model prefers. Returns the crop canvas. */
export function cropRegion(bitmap, region) {
  const b = region.box;
  const pad = 4;
  const w = Math.max(8, Math.round(b.x1 - b.x0) + pad * 2);
  const h = Math.max(8, Math.round(b.y1 - b.y0) + pad * 2);
...
/** Shared inline styles for the CRAFT text-detection widgets. Injected once per page. */
export const CRAFT_CSS = `
...
.heat-pair { display: flex; flex-wrap: wrap; gap: 1rem; }
.heat-pair figure { flex: 1 1 260px; min-inline-size: 0; margin: 0; }
.heat-pair figcaption { font-size: .78rem; color: var(--muted); margin-top: .3rem; }
...
.crop-strip { display: flex; flex-wrap: wrap; gap: .6rem; margin: .6rem 0; }
.crop-cell { border: 1px solid var(--border); border-radius: 6px; padding: .4rem; background: var(--bg-raised); max-inline-size: 220px; }
.crop-cell canvas { display: block; max-inline-size: 200px; block-size: auto; border-radius: 4px; }
.crop-cell .ocr-text { font-family: var(--font-mono); font-size: .8rem; margin-top: .35rem; word-break: break-word; }

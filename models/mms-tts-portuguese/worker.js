// MMS-TTS Portuguese (VITS) worker — end-to-end Portuguese speech synthesis off the main thread so the control
...
// is no separate vocoder: Portuguese text → language-specific token ids → (flows + duration) → 16 kHz
// waveform, all in one network. Nothing here is faked.
//
// This demo is the PORTUGUESE checkpoint (Xenova/mms-tts-por) — a materially DISTINCT model from the built
// English MMS-TTS demo: its own Portuguese character vocabulary and its own VITS weights, trained on Portuguese
// speech, so it renders Portuguese orthography (accented vowels, ç, the ã/õ nasals and Portuguese phonotactics) that the English
// checkpoint cannot. The default modelId is the Portuguese one; the worker still accepts any mms-tts-<iso>
// id so a page can compare a second language if it wants.
//
// Because the duration predictor is stochastic, generating the same Portuguese text twice yields DIFFERENT
// timing/prosody — that variation is real and is what the "stochastic prosody" surface visualises.

import { TRANSFORMERS_URL } from "/web-ai-showcase/lib/webai.js";

const DEFAULT_MODEL = "Xenova/mms-tts-por";

// Front-end helpers shared by every GFPGAN face-restoration page. Keeps pages thin: owns the worker
// handshake (transferring ImageBitmaps so nothing is copied), turns files / gallery images into
// ImageBitmaps, can crop a face region and synthesise a degraded (small + soft + compressed) version so
// you can watch GFPGAN rebuild it, and renders the restored result + an accessible before/after reveal
// slider. ALL inference AND the dense output composite live in worker.js (off the main thread, raw ONNX
// Runtime Web). Privacy by construction: the face and every restored pixel never leave the device.

const WORKER_URL = "/web-ai-showcase/models/gfpgan-face-restoration/worker.js";
...
export class FaceRestoreEngine {
  ...
  /** Restore a face ImageBitmap (transferred → zero-copy). Returns
...
// Optionally crop a square face region from a larger photo, given a normalised box {x,y,w,h} in [0,1].
// Returns a fresh ImageBitmap of just the face (or the whole image if no box).
export async function cropFace(bitmap, box) {
  if (!box) return createImageBitmap(bitmap);
  const sx = Math.round(box.x * bitmap.width), sy = Math.round(box.y * bitmap.height);
  const sw = Math.round(box.w * bitmap.width), sh = Math.round(box.h * bitmap.height);
  const c = new OffscreenCanvas(sw, sh);
  c.getContext("2d").drawImage(bitmap, sx, sy, sw, sh, 0, 0, sw, sh);
  return c.transferToImageBitmap();
}

// Synthesise a DEGRADED face so the demo can show GFPGAN rebuilding it. Shrinks to `small` px (losing
// detail), optionally re-encodes as a low-quality JPEG (blocky artefacts), and adds a soft blur — the
// kind of degradation old scans and low-res crops carry, which GFPGAN was trained to undo. Returns a
// fresh ImageBitmap. Honest by design — the user chooses the degradation and watches it come back; real
// old/degraded portraits can be uploaded instead.
export async function degradeFace(bitmap, { small = 96, jpeg = 30, blur = 1 } = {}) {
  const long = Math.max(bitmap.width, bitmap.height);
  // down to a tiny thumbnail, then back up to ~512 with the detail already lost
  const s = Math.min(small / long, 1);
  const w = Math.max(1, Math.round(bitmap.width * s)),
    h = Math.max(1, Math.round(bitmap.height * s));
  const down = new OffscreenCanvas(w, h);
  const dctx = down.getContext("2d");
  dctx.imageSmoothingQuality = "high";
  dctx.drawImage(bitmap, 0, 0, w, h);
  let src = down;
  if (jpeg > 0) {
    const q = Math.max(0.05, Math.min(0.95, 1 - jpeg / 100));
    const blob = await down.convertToBlob({ type: "image/jpeg", quality: q });
    src = await createImageBitmap(blob);
  }
  const up = new OffscreenCanvas(512, 512);
  const uctx = up.getContext("2d");
  uctx.imageSmoothingQuality = "high";
  if (blur > 0) uctx.filter = `blur(${blur}px)`;
  uctx.drawImage(src, 0, 0, w, h, 0, 0, 512, 512);
  uctx.filter = "none";
  return up.transferToImageBitmap();
}
...
export class RevealCompare {
  ...
  set(beforeBmp, afterBmp, maxW = 512) {
    drawBitmapFit(this.before, beforeBmp, maxW);
    this.after.width = this.before.width;
    this.after.height = this.before.height;
    const ctx = this.after.getContext("2d");
    ctx.imageSmoothingQuality = "high";
    ctx.drawImage(afterBmp, 0, 0, this.after.width, this.after.height);
    this._apply();
  }
}

/** Shared inline styles for the face-restoration widgets. Structural only — colours from design-system vars. */
export const RESTORE_CSS = `
...
.reveal { position: relative; inline-size: 100%; max-inline-size: 512px; border-radius: 6px; overflow: hidden; background: var(--bg-raised); touch-action: pan-y; }
...
.reveal-labels { display: flex; justify-content: space-between; max-inline-size: 512px; font-family: var(--font-mono); font-size: .72rem; color: var(--muted); margin-top: .35rem; }

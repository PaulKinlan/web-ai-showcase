...
- **Verify runnability first; don't churn on known-blocked builds.** Confirm a REAL build runs
  ...
  **git** (no `git` class; no `Xenova/git-*` ONNX), **vilt** (no `vilt` class / no
  ...
  `Unsupported model type: internvl`), **arabic-bert / arabic-MLM** (canonical Arabic masked-LMs
  ...
  **subjectivity/propaganda/humor/text-chunking detection** (dedicated fine-tunes exist but ALL are
  ...
  plant-disease ONNX — canonical MobileNetV2 is .bin-only; only 'plant' ONNX is houseplant-SPECIES
  not disease; don't relabel), **nllb-clip** (no `NLLBCLIPModel` class — custom open_clip; Immich
  ONNX is a bespoke split layout not an HF pipeline; don't relabel SigLIP2/Jina-CLIP), **spanbert**
  (released weights are a bare encoder w/ NO MLM head → `logits===undefined`; every ONNX is

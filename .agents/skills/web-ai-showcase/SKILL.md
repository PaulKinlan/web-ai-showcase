...
- **Verify runnability first; don't churn on known-blocked builds.** Confirm a REAL build runs
  ...
  **git** (no `git` class; no `Xenova/git-*` ONNX), **vilt** (no `vilt` class / no
  ...
  `Unsupported model type: internvl`), **nllb-clip** (no `NLLBCLIPModel` class — custom open_clip;
  Immich ONNX is a bespoke split layout not an HF pipeline; don't relabel SigLIP2/Jina-CLIP),
  **spanbert** (released weights are a bare encoder w/ NO MLM head → `logits===undefined`; every
  ONNX is feature-extraction/QA; same as scibert/electra), **mxbai-rerank-v2** (v2 is
  Qwen2ForCausalLM decoder-reranker; the WASM-q8 export is head-less [no lm_head → can't compute 1/0
  logit]; don't relabel v1), **boolean-qa/boolq** (runnable BoolQ fine-tunes ship no ONNX; the only
  .onnx exports are head-less encoders [no classification head → logits undefined]; don't relabel
